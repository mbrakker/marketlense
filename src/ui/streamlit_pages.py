...
def _try_read_app_config() -> tuple[Any | None, str | None]:
    try:
        response = read_app_config(AppConfigReadRequest(schema_version="1.0", path=""), _ctx("read_app_config"))
    except Exception as exc:  # pragma: no cover - runtime safeguard for UI
        return None, str(exc)
    return response, None


def _try_write_app_config(content: str, *, make_backup: bool) -> tuple[Any | None, str | None]:
    try:
...
def _storage_health(settings: Any) -> list[dict[str, Any]]:
    response = collect_storage_health(
        StorageHealthRequest(
            schema_version="1.0",
            targets=[
                StorageTarget(schema_version="1.0", name="output_dir", path=settings.output_dir),
                StorageTarget(schema_version="1.0", name="cache_dir", path=settings.cache_dir),
                StorageTarget(schema_version="1.0", name="state_db", path=settings.state_db),
                StorageTarget(schema_version="1.0", name="reports_db", path=settings.reports_db),
            ],
        ),
        _ctx("storage_health"),
    )
...
def _recent_validation_files(output_dir: str) -> list[dict[str, Any]]:
    response = summarize_validation_artifacts(
        ValidationArtifactSummaryRequest(schema_version="1.0", output_dir=output_dir, limit=200),
...
def _load_processed_rows(settings: Any) -> list[dict[str, Any]]:
    response = load_state_rows(
        StateRowsLoadRequest(schema_version="1.0", state_db=settings.state_db, kind="processed", limit=1000),
...
def _load_published_rows(settings: Any) -> list[dict[str, Any]]:
    response = load_state_rows(
        StateRowsLoadRequest(schema_version="1.0", state_db=settings.state_db, kind="published", limit=1000),
...
def _load_ledger_entries(ledger_path: str, *, limit: int = 2000) -> list[dict[str, Any]]:
    response = load_ledger_entries(
        LedgerEntriesLoadRequest(schema_version="1.0", ledger_path=ledger_path, limit=limit),
...
def _render_structured_config_form(config_payload: dict[str, Any], *, editor_key: str) -> None:
    ...
    with st.form("app_yaml_structured_form", border=False):
        ...
        with core_col1:
            schema_version = st.text_input("Schema Version", value=_as_str(working.get("schema_version"), "1.0"))
        with core_col2:
            ingest_openai_model = st.text_input("Ingest OpenAI Model", value=_as_str(ingest.get("openai_model"), "gpt-5-mini"))
        with core_col3:
            rank_model = st.text_input("Rank Model", value=_as_str(rank.get("model"), ingest_openai_model))

        with st.expander("Paths", expanded=True):
            p1, p2 = st.columns(2, gap="large")
            with p1:
                path_output_dir = st.text_input("Output Dir", value=_as_str(paths.get("output_dir"), "./out"))
                path_state_db = st.text_input("State DB", value=_as_str(paths.get("state_db"), "./state/index.sqlite"))
                path_category_mappings = st.text_input(
                    "Category Mappings YAML",
                    value=_as_str(paths.get("category_mappings"), "./src/config/category-mappings.yaml"),
                )
                path_cover_styles = st.text_input(
                    "Cover Styles YAML",
                    value=_as_str(paths.get("cover_styles"), "./src/config/cover-styles.yaml"),
                )
            with p2:
                path_cache_dir = st.text_input("Cache Dir", value=_as_str(paths.get("cache_dir"), "./cache"))
                path_reports_db = st.text_input("Reports DB", value=_as_str(paths.get("reports_db"), "./state/reports.sqlite"))
                path_html_tag_acronyms = st.text_input(
                    "HTML Tag Acronyms YAML",
                    value=_as_str(paths.get("html_tag_acronyms"), "./src/config/html-tag-acronyms.yaml"),
                )
                path_ingest_lock = st.text_input("Ingest Lock Path", value=_as_str(paths.get("ingest_lock"), "./state/ingest.lock"))

        with st.expander("Ingest Core", expanded=True):
            i1, i2, i3 = st.columns(3, gap="large")
            with i1:
                ingest_google_sa_path = st.text_input("Google SA Path", value=_as_str(ingest.get("google_sa_path"), "./sa.json"))
                ingest_temperature = st.number_input("Ingest Temperature", value=_as_float(ingest.get("temperature"), 1.0), step=0.1, format="%.3f")
                ingest_batch_limit = st.number_input("Batch Limit", value=_as_int(ingest.get("batch_limit"), 20), min_value=1, step=1)
                ingest_worker_limit = st.number_input("Worker Limit", value=_as_int(ingest.get("worker_limit"), 2), min_value=1, step=1)
            with i2:
                ingest_gdrive_folder_id = st.text_input("GDrive Folder ID", value=_as_str(ingest.get("gdrive_folder_id"), ""))
        ...
        with st.expander("Drive", expanded=False):
            ...
            with d2:
                drive_id = st.text_input("Drive ID (optional)", value=_as_str(drive.get("drive_id"), ""))
                drive_list_mode = st.selectbox(
                    "Drive List Mode",
                    options=["metadata", "full"],
                    index=0 if _as_str(drive.get("list_mode"), "metadata").strip().lower() != "full" else 1,
                )

        with st.expander("PDF Text & Contents", expanded=False):
            pt1, pt2 = st.columns(2, gap="large")
            with pt1:
                pdf_text_max_pages = st.number_input("PDF Text Max Pages", value=_as_int(pdf_text.get("max_pages"), 5), min_value=1, step=1)
                pdf_text_max_chars = st.number_input("PDF Text Max Chars", value=_as_int(pdf_text.get("max_chars"), 80000), min_value=1000, step=1000)
                pdf_text_min_density = st.number_input("PDF Text Min Density", value=_as_float(pdf_text.get("min_density"), 250.0), min_value=0.0, step=10.0)
                pdf_text_sample_pages = st.number_input("PDF Text Sample Pages", value=_as_int(pdf_text.get("sample_pages"), 3), min_value=1, step=1)
            with pt2:
                contents_max_pages = st.number_input("Contents Max Pages", value=_as_int(contents_page.get("max_pages"), 8), min_value=1, step=1)
                contents_min_headings = st.number_input("Contents Min Headings", value=_as_int(contents_page.get("min_headings"), 3), min_value=1, step=1)
                contents_preview_enabled = st.checkbox("Contents Preview Enabled", value=_as_bool(contents_page.get("preview_enabled"), True))
                contents_render_dpi = st.number_input("Contents Render DPI", value=_as_int(contents_page.get("render_dpi"), 144), min_value=72, step=1)
            keywords_default = contents_page.get("keywords") if isinstance(contents_page.get("keywords"), list) else []
            contents_keywords_text = st.text_area(
                "Contents Keywords (one per line)",
                value="\n".join(str(item).strip() for item in keywords_default if str(item).strip()),
                height=100,
            )
        ...
        with st.expander("Validation + Parallelism", expanded=False):
            ...
            with vp1:
                validation_data_gap_policy = st.selectbox(
                    ...
                    options=["warn", "fail"],
                    index=0 if _as_str(ingest_validation.get("data_gap_policy"), "warn").strip().lower() != "fail" else 1,
                )
            with vp2:
        ...
        with st.expander("OpenAI Namespace Model Overrides", expanded=False):
            openai_rows = [{"namespace": key, "model": value} for key, value in sorted(openai_models.items())]
        ...
        with st.expander("Rank", expanded=False):
            ...
            with r1:
                rank_temperature = st.number_input("Rank Temperature", value=_as_float(rank.get("temperature"), 1.0), step=0.1, format="%.3f")
                rank_timeout_seconds = st.number_input("Rank Timeout Seconds", value=_as_float(rank.get("timeout_seconds"), 600.0), min_value=1.0, step=1.0, format="%.1f")
                rank_seed_text = st.text_input(
                    "Rank Seed (blank for null)",
                    value="" if rank.get("seed") in {None, ""} else _as_str(rank.get("seed")),
                )
                rank_max_candidates = st.number_input("Rank Max Candidates", value=_as_int(rank.get("max_candidates"), 40), min_value=1, step=1)
                rank_selected_max = st.number_input("Rank Selected Max", value=_as_int(rank.get("selected_max"), 5), min_value=1, step=1)
            with r2:
                rank_min_overall_score = st.number_input("Rank Min Overall Score", value=_as_int(rank.get("min_overall_score"), 78), min_value=0, max_value=100, step=1)
                rank_min_quality_score = st.number_input("Rank Min Quality Score", value=_as_int(rank.get("min_quality_score"), 75), min_value=0, max_value=100, step=1)
                rank_min_insight_score = st.number_input("Rank Min Insight Score", value=_as_int(rank.get("min_insight_score"), 75), min_value=0, max_value=100, step=1)
                rank_min_data_score = st.number_input("Rank Min Data Score", value=_as_int(rank.get("min_data_score"), 70), min_value=0, max_value=100, step=1)
            with r3:
                rank_crop_refine_enabled = st.checkbox("Crop Refine Enabled", value=_as_bool(rank.get("crop_refine_enabled"), True))
                rank_crop_refine_mode = st.selectbox(
                    "Crop Refine Mode",
                    options=["adaptive", "always", "off"],
                    index=["adaptive", "always", "off"].index(_as_str(rank.get("crop_refine_mode"), "adaptive").strip().lower())
                    if _as_str(rank.get("crop_refine_mode"), "adaptive").strip().lower() in {"adaptive", "always", "off"}
                    else 0,
                )
                rank_crop_refine_page_dpi = st.number_input("Crop Refine Page DPI", value=_as_int(rank.get("crop_refine_page_dpi"), 110), min_value=72, step=1)
                ...
                rank_crop_refine_timeout_seconds = st.number_input(
                    ...
                    value=_as_float(rank.get("crop_refine_timeout_seconds"), _as_float(rank.get("timeout_seconds"), 600.0)),
                    min_value=1.0,
                    step=1.0,
                    format="%.1f",
                )
        ...
        with st.expander("Publish", expanded=False):
            ...
            with pub1:
                wp_site_url = st.text_input("WordPress Site URL", value=_as_str(wp.get("site_url"), ""))
                wp_username = st.text_input("WordPress Username", value=_as_str(wp.get("username"), ""))
                wp_post_status = st.selectbox(
                    "WordPress Post Status",
                    options=["publish", "draft", "pending", "private"],
                    index=0 if _as_str(wp.get("post_status"), "publish") not in {"publish", "draft", "pending", "private"} else ["publish", "draft", "pending", "private"].index(_as_str(wp.get("post_status"), "publish")),
                )
            with pub2:
                publish_validation_policy = st.selectbox(
                    "Publish Validation Policy",
                    options=["block", "warn"],
                    index=0 if _as_str(publish_validation.get("policy"), "block").strip().lower() != "warn" else 1,
                )
        ...
        with st.expander("Analysis & Cost", expanded=False):
            ...
            with ac2:
                cost_daily_path = st.text_input("Cost Daily Path", value=_as_str(cost.get("daily_path"), "./out/cost-daily.json"))
            pricing_rows = []
            for model, model_prices in sorted(pricing.items()):
                ...
                pricing_rows.append({
                    "model": str(model),
                    "input_tokens_per_1k_usd": _as_float(model_prices.get("input_tokens_per_1k_usd"), 0.0),
                    "output_tokens_per_1k_usd": _as_float(model_prices.get("output_tokens_per_1k_usd"), 0.0),
                    "tool_call_usd": _as_float(model_prices.get("tool_call_usd"), 0.0),
                })
        ...
        apply_clicked = st.form_submit_button("Apply Structured Changes To YAML", type="secondary", use_container_width=True)

    if not apply_clicked:
        return

    errors: list[str] = []
    ingest_seed = _optional_int_from_text(ingest_seed_text, field="Ingest seed", errors=errors)
    rank_seed = _optional_int_from_text(rank_seed_text, field="Rank seed", errors=errors)
    ...
    st.session_state["app_yaml_notice"] = "Structured changes applied to YAML editor. Open 'YAML Editor' tab and click Save."
    st.rerun()
...
def _render_settings_and_prompts(
    ...
    with filters:
        st.caption("Edit every key in `src/config/app.yaml`, save from UI, and inspect prompt/runtime state.")
    ...
    with main_col:
        ...
        with tab_form:
            st.caption(f"Form source: {form_payload_source}. Apply changes to update the YAML editor.")
        ...
        with tab_editor:
            ...
            if save_clicked:
                save_response, save_error = _try_write_app_config(editor_text, make_backup=make_backup)
                if save_error:
                    ...
                    detail = f" Backup: `{save_response.backup_path}`." if save_response.backup_path else ""
                    st.success(f"Saved `{save_response.path}`.{detail}")
                    candidate_settings, candidate_error = _try_load_settings()
                    if candidate_error:
                        st.warning(f"Saved, but runtime settings are currently invalid: {candidate_error}")
        ...
        with tab_runtime:
            ...
            st.code("\n".join(str(key) for key in config_doc.payload.keys()) if config_doc else "")
...
def _render_system_and_storage(settings: Any) -> None:
    ...
    path_checks_response = collect_directory_counts(
        DirectoryCountsRequest(
            schema_version="1.0",
            checks=[
                DirectoryCountCheck(schema_version="1.0", name="HTML", root_dir=settings.output_dir, glob_pattern="*.html", recursive=False, include_dirs=False),
                DirectoryCountCheck(schema_version="1.0", name="report_analysis dirs", root_dir=settings.output_dir, glob_pattern="report_analysis", recursive=True, include_dirs=True),
                DirectoryCountCheck(schema_version="1.0", name="assets dirs", root_dir=settings.output_dir, glob_pattern="assets", recursive=True, include_dirs=True),
                DirectoryCountCheck(schema_version="1.0", name="candidates dirs", root_dir=settings.output_dir, glob_pattern="candidates", recursive=True, include_dirs=True),
                DirectoryCountCheck(schema_version="1.0", name="slices dirs", root_dir=settings.output_dir, glob_pattern="slices", recursive=True, include_dirs=True),
                DirectoryCountCheck(schema_version="1.0", name="thumbs dirs", root_dir=settings.output_dir, glob_pattern="thumbs", recursive=True, include_dirs=True),
            ],
            limit=5000,
        ),
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        if settings_error:
            st.error("Runtime config is invalid. Open Settings & Prompts to fix app.yaml.")
        section = st.radio(
            "Navigation",
    ...
    if settings is None and section != "Settings & Prompts":
        ...
        _render_settings_and_prompts(settings, publish_settings, publish_error, settings_error)
        return
