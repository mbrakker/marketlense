...
class AcquisitionAuditCandidateResult:
    ...
    publisher_insights_url: str = field(
        metadata={"doc": "Publisher insights URL used for the discovery run that surfaced this candidate."}
    )
    publisher_discovery_route_kind: str = field(
        metadata={"doc": "Discovery route kind used for the publisher run that surfaced this candidate."}
    )
    publisher_recommended_discovery_route_kind: str = field(
        metadata={"doc": "Recommended future discovery route kind for this publisher based on the run-quality summary."}
    )
    ...
    source_page_urls: List[str] = field(
        metadata={"doc": "Distinct inventory page URLs where the candidate was observed during discovery."}
    )
    discovery_provenances: List[str] = field(
        metadata={"doc": "Distinct provenance labels describing how the candidate was discovered."}
    )
    acquisition_route_kind: str = field(
        metadata={"doc": "Observed acquisition route kind for the candidate: pdf_download, email_delivery, failed_retryable, or failed_permanent."}
    )
    acquisition_outcome: str = field(
        metadata={"doc": "Observed acquisition outcome for the candidate, for example downloaded, email_requested, email_required, failed_retryable, or failed_permanent."}
    )
    recommended_report_flow: str = field(
        metadata={"doc": "Recommended future automation flow for this report candidate."}
    )
    ...
    acquisition_route_summary: Optional[str] = field(
        default=None,
        metadata={"doc": "Observed acquisition route summary when the download flow reached a terminal route."},
    )
    acquisition_final_page_url: Optional[str] = field(
        default=None,
        metadata={"doc": "Final browser URL observed during acquisition when available."},
    )
    encountered_form_fields: List[str] = field(
        default_factory=list,
        metadata={"doc": "Distinct form field labels encountered during acquisition when available."},
    )
    downloaded_file_path: Optional[str] = field(
        default=None,
        metadata={"doc": "Absolute downloaded file path when the report was acquired locally."},
    )
...
class AcquisitionAuditPublisherSummary:
    ...
    recommended_publisher_flow: str = field(
        metadata={"doc": "Recommended future acquisition strategy for this publisher based on candidate audit outcomes."}
    )
    recommendation_reason: str = field(
        metadata={"doc": "Short human-readable reason for the publisher-level recommendation."}
    )
    current_candidate_count: int = field(
        metadata={"doc": "Number of current normalized candidates surfaced by discovery for this publisher."}
    )
    downloaded_count: int = field(
        metadata={"doc": "Number of candidates whose acquisition flow produced a local download."}
    )
    email_requested_count: int = field(
        metadata={"doc": "Number of candidates whose acquisition flow successfully submitted an email-delivery request."}
    )
    email_required_count: int = field(
        metadata={"doc": "Number of candidates whose acquisition flow requires email completion before automation can finish."}
    )
    ...
    discovery_provenance_counts: dict[str, int] = field(
        ...
        metadata={"doc": "Counts of discovery provenance labels observed across the publisher's current candidates."},
    )
    acquisition_route_counts: dict[str, int] = field(
        default_factory=dict,
        metadata={"doc": "Counts of acquisition route kinds observed across the publisher's audited candidates."},
    )
    acquisition_outcome_counts: dict[str, int] = field(
        default_factory=dict,
        metadata={"doc": "Counts of acquisition outcomes observed across the publisher's audited candidates."},
    )
    error_code: Optional[str] = field(
        default=None,
        metadata={"doc": "Typed AppError code when the publisher-level discovery phase failed."},
    )
    error_message: Optional[str] = field(
        default=None,
...
class AcquisitionAuditBatchRequest:
    ...
    publisher_inventory_settings: PublisherInventorySettings = field(
        metadata={"doc": "Loaded publisher inventory settings used for the discovery phase of the audit."}
    )
    browser_download_settings: BrowserDownloadSettings = field(
        metadata={"doc": "Loaded browser download settings used for the acquisition phase of the audit."}
    )
    output_dir: str = field(
        metadata={"doc": "Root output directory where audit artifacts should be written."}
    )
    delivery_email: Optional[str] = field(
        default=None,
        metadata={"doc": "Optional delivery email used when report acquisition reaches an email-gated route."},
    )
    ...
    candidate_limit_per_publisher: Optional[int] = field(
        default=None,
        metadata={"doc": "Optional maximum number of current candidates to audit per publisher."},
    )
...
class AcquisitionAuditBatchResult:
    ...
    output_path: str = field(
        metadata={"doc": "Filesystem path of the written acquisition-audit JSON artifact."}
    )
