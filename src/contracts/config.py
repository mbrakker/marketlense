...
class AppSettings:
    ...
    rank_selected_max: int = field(default=5, metadata={"doc": "Maximum number of final ranked candidates to include in HTML figure gallery."})
    rank_min_overall_score: int = field(default=78, metadata={"doc": "Minimum overall rank score required for a candidate to pass threshold gate."})
    rank_min_quality_score: int = field(default=75, metadata={"doc": "Minimum quality score required for a candidate to pass threshold gate."})
    rank_min_insight_score: int = field(default=75, metadata={"doc": "Minimum insightfulness score required for a candidate to pass threshold gate."})
    rank_min_data_score: int = field(default=70, metadata={"doc": "Minimum data-density score required for a candidate to pass threshold gate."})
    crop_refine_enabled: bool = field(default=True, metadata={"doc": "Whether GPT-assisted crop refinement is enabled for ambiguous candidates."})
    crop_refine_mode: str = field(default="adaptive", metadata={"doc": "Crop refinement mode: adaptive|always|off."})
    crop_refine_page_dpi: int = field(default=110, metadata={"doc": "DPI used when rendering page context images for crop refinement."})
    crop_refine_temperature: float = field(default=0.0, metadata={"doc": "Sampling temperature for crop refinement model calls."})
    crop_refine_timeout_seconds: float = field(default=600.0, metadata={"doc": "Timeout in seconds for crop refinement model calls."})
