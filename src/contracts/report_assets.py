...
class CropRefinePageRenderRequest:
    schema_version: str = field(metadata={"doc": "Crop-refine page render request schema version."})
    pdf_path: str = field(metadata={"doc": "Filesystem path to the PDF."})
    ...
    dpi: int = field(default=110, metadata={"doc": "Render DPI for page context image."})
    pdf_context: Optional[PdfContext] = field(default=None, metadata={"doc": "Optional pre-opened PDF context to reuse handles."})


@dataclass(frozen=True)
class CropRefinePageRenderResponse:
    schema_version: str = field(metadata={"doc": "Crop-refine page render response schema version."})
    ...
    scale_x: float = field(metadata={"doc": "Horizontal conversion scale from PDF points to image pixels."})
    scale_y: float = field(metadata={"doc": "Vertical conversion scale from PDF points to image pixels."})


@dataclass(frozen=True)
class CropRefineBBoxApplyRequest:
    schema_version: str = field(metadata={"doc": "Crop-refine bbox apply request schema version."})
    pdf_path: str = field(metadata={"doc": "Filesystem path to the PDF."})
    page: int = field(metadata={"doc": "Zero-based page index for bbox clamping."})
    bbox: Tuple[float, float, float, float] = field(metadata={"doc": "Proposed PDF-space bbox to clamp."})
    pdf_context: Optional[PdfContext] = field(default=None, metadata={"doc": "Optional pre-opened PDF context to reuse handles."})


@dataclass(frozen=True)
class CropRefineBBoxApplyResponse:
    schema_version: str = field(metadata={"doc": "Crop-refine bbox apply response schema version."})
    page: int = field(metadata={"doc": "Zero-based page index for clamped bbox."})
    bbox: Tuple[float, float, float, float] = field(metadata={"doc": "Clamped and normalized PDF-space bbox."})


@dataclass(frozen=True)
class CropRefineCandidate:
    schema_version: str = field(metadata={"doc": "Crop-refine candidate schema version."})
    ...
    bbox: Tuple[float, float, float, float] = field(metadata={"doc": "Candidate bbox in PDF-space coordinates."})
    caption: str = field(default="", metadata={"doc": "Candidate caption/title text if available."})
    preview_text: str = field(default="", metadata={"doc": "Candidate preview text snippet."})
    meta: Dict[str, Any] = field(default_factory=dict, metadata={"doc": "Candidate metadata with heuristic signals."})
...
class CropRefineResult:
    ...
    id: str = field(metadata={"doc": "Candidate identifier."})
    is_valid_candidate: bool = field(metadata={"doc": "Whether candidate is valid for final HTML figure output."})
    refined_bbox: Tuple[float, float, float, float] = field(metadata={"doc": "Refined PDF-space bbox."})
    include_title: bool = field(metadata={"doc": "Whether title/caption should be included in final crop."})
    include_note_if_present: bool = field(metadata={"doc": "Whether note/source line should be included when attached."})
    confidence: float = field(metadata={"doc": "Model confidence score between 0 and 1."})
    reason: str = field(default="", metadata={"doc": "Model-provided reason for decision."})
...
class CropRefineRequest:
    ...
    temperature: float = field(metadata={"doc": "Sampling temperature for crop refinement."})
    api_key: str = field(metadata={"doc": "OpenAI API key (secret, loaded from env)."})
    page_image_path: str = field(metadata={"doc": "Filesystem path to rendered page context image."})
    page: int = field(metadata={"doc": "Zero-based page index for supplied image context."})
    page_width: float = field(metadata={"doc": "Original PDF page width in points."})
    page_height: float = field(metadata={"doc": "Original PDF page height in points."})
    candidates: List[CropRefineCandidate] = field(metadata={"doc": "Candidates to evaluate and refine on this page."})
    seed: Optional[int] = field(default=None, metadata={"doc": "Optional deterministic seed for crop refinement."})
...
class CropRefineResponse:
    schema_version: str = field(metadata={"doc": "Crop-refine response schema version."})
    results: List[CropRefineResult] = field(metadata={"doc": "Crop refinement decisions for submitted candidates."})
