...
class CandidateExtractRequest(SemanticIdContract):
    ...
    subdir: str = field(default="candidates", metadata={"doc": "Subdirectory for cropped images."})
    save_crops: bool = field(default=True, metadata={"doc": "Whether to crop and save candidate images."})
...
class CandidateExtractOutcome(SemanticIdContract):
    ...
    candidate_count: int = field(metadata={"doc": "Total number of candidates extracted."})
    ...
    crop_count: int = field(metadata={"doc": "Number of cropped candidate images saved."})
    crop_paths: List[str] = field(metadata={"doc": "Relative paths to cropped candidate images."})
    error: Optional[str] = field(default=None, metadata={"doc": "Error message when extraction fails."})
