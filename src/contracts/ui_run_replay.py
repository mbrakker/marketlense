...
class UiRunArtifactFingerprint:
    ...
    exists: bool = field(
        metadata={"doc": "Whether the artifact existed when the fingerprint was captured."}
    )
    size_bytes: int = field(
        metadata={"doc": "Artifact size in bytes when the artifact existed, else zero."}
    )
    sha256: str = field(
        metadata={"doc": "SHA-256 digest for the artifact bytes when the artifact existed, else empty."}
    )
...
class UiRunExecutionResponse(SemanticIdContract):
    ...
    status: str = field(
        metadata={"doc": "Execution status for the worker payload: succeeded or failed."}
    )
    result_summary: dict[str, Any] = field(
        metadata={"doc": "Workflow-specific summary fields produced by the execution attempt."}
    )
    artifact_paths: list[str] = field(
        metadata={"doc": "Artifact paths produced by the execution attempt when known."}
    )
    config_snapshot: dict[str, Any] = field(
        metadata={"doc": "Sanitized deterministic configuration snapshot used for replay gating."}
    )
...
class UiRunReplayManifest(SemanticIdContract):
    ...
    config_snapshot: dict[str, Any] = field(
        metadata={"doc": "Sanitized deterministic configuration snapshot recorded for replay."}
    )
    config_fingerprint: str = field(
        metadata={"doc": "SHA-256 digest of the sanitized configuration snapshot."}
    )
    ...
    source_tree_fingerprint: str = field(
        metadata={"doc": "SHA-256 digest for the Python source tree used by the original run."}
    )
    ...
    artifact_fingerprints: list[UiRunArtifactFingerprint] = field(
        metadata={"doc": "Fingerprints for artifacts recorded by the original worker attempt."}
    )
    error_code: str = field(
        default="",
    ...
    error_message: str = field(
        default="",
        metadata={"doc": "Human-readable failure detail recorded for failed original runs."},
    )
...
class UiRunReplayCaptureRequest(SemanticIdContract):
    ...
    run_id: RunId = field(metadata={"doc": "UI run identifier whose manifest should be written."})
    run_type: str = field(metadata={"doc": "Stable run type to record in the manifest."})
    status: str = field(
...
class UiRunReplayReadRequest(SemanticIdContract):
    ...
    run_id: RunId = field(metadata={"doc": "UI run identifier whose replay manifest should be read."})
...
class UiRunArtifactFingerprintRequest(SemanticIdContract):
    ...
    artifact_paths: list[str] = field(
        metadata={"doc": "Artifact paths that should be fingerprinted for replay comparison."}
    )
...
class UiRunWorkspaceFingerprintResponse:
    ...
    source_tree_root: str = field(
        metadata={"doc": "Resolved root directory used to fingerprint Python source files."}
    )
    source_tree_fingerprint: str = field(
...
class UiRunReplayDelta:
    schema_version: str = field(
        metadata={"doc": "UI-run replay delta schema version."}
    )
    field_name: str = field(
        metadata={"doc": "Stable field name being compared between original and replay outputs."}
    )
    matches: bool = field(
...
class UiRunReplayReport(SemanticIdContract):
    ...
    run_id: RunId = field(metadata={"doc": "Original UI run identifier being replayed."})
    replayed_at_utc: str = field(
        metadata={"doc": "UTC timestamp when the replay finished and the report was generated."}
    )
    replay_status: str = field(
        metadata={"doc": "Terminal replay status after the deterministic replay attempt."}
    )
    source_fingerprint_match: bool = field(
        metadata={"doc": "Whether the current Python source tree matched the original manifest fingerprint."}
    )
    prompt_fingerprint_match: bool = field(
        metadata={"doc": "Whether the current prompt tree matched the original manifest fingerprint."}
    )
    config_fingerprint_match: bool = field(
        metadata={"doc": "Whether the current sanitized configuration snapshot matched the original manifest fingerprint."}
    )
    deltas: list[UiRunReplayDelta] = field(
        metadata={"doc": "Field-by-field output comparisons between the original run and the replay attempt."}
    )
    matched: bool = field(
        metadata={"doc": "Whether the replay matched the original environment gates and output deltas exactly."}
    )
...
class UiRunReplayResponse:
    ...
    manifest_path: str = field(
        metadata={"doc": "Filesystem path to the loaded replay manifest for the original run."}
    )
    report_path: str = field(
        metadata={"doc": "Filesystem path where the replay comparison report was written."}
    )
    report: UiRunReplayReport = field(
