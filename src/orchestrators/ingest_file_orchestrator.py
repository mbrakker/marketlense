...
class IngestFileDependencies:
    ...
    sidecar_md5_for_stat: Callable[[dict, Optional[int], Optional[float]], Optional[str]]
    ensure_file_name: Callable[[DriveFile, IngestSettings, RunContext], DriveFile]
    write_md5_sidecar: Callable[[str, DriveFile, Optional[str], Optional[int], Optional[float], RunContext], None]
    existing_report_html: Callable[[DriveFile, str, IngestSettings, RunContext], Optional[str]]
    ...
    run_report_pipeline: Callable[[DriveFile, str, IngestSettings, Optional[str], RunContext], IngestOutcome]
...
def run_ingest_file(
    ...
    *,
    ...
    try:
        ...
        cache_reason = ""
        sidecar_used = False
        stat_resp = dependencies.file_stat(FileStatRequest(schema_version="1.0", path=cache_path), file_ctx)
        if stat_resp.exists:
            sidecar_payload = dependencies.load_md5_sidecar(sidecar_path, file.file_id, file_ctx)
            md5 = dependencies.sidecar_md5_for_stat(sidecar_payload, stat_resp.size_bytes, stat_resp.mtime_utc)
            if md5:
                sidecar_used = True
                logger.info(log_event(
                    file_ctx,
        ...
        if md5 and md5 != state_checked_md5 and dependencies.should_skip(file, md5, settings.state_db, file_ctx):
            logger.info(log_event(
                file_ctx,
                role="orchestrator",
        ...
        if md5 and md5 != report_checked_md5:
            existing_html = dependencies.existing_report_html(file, md5, settings, file_ctx)
