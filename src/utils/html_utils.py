...
_INTERNAL_PUBLICATION_SCRIPT_RX = re.compile(
    r'<script\b[^>]*\bdata-market-lense-(?:publish-entity|cross-report-metadata)=["\']true["\'][^>]*>.*?</script>',
    re.IGNORECASE | re.DOTALL,
)
_HIDDEN_FILE_ID_MARKER_RX = re.compile(
    r'<p\b[^>]*\bhidden(?:=[^\s>]*)?[^>]*>\s*Drive fileId:\s*[^<]+</p>',
    re.IGNORECASE | re.DOTALL,
)
_PUBLICATION_FILE_ID_TEXT_RX = re.compile(
    r"Drive fileId:\s*[A-Za-z0-9._:-]+",
    re.IGNORECASE,
)
_INTERNAL_FILE_ID_FOOTER_RX = re.compile(
    r'<footer\b[^>]*\bclass=["\'][^"\']*\bfooter\b[^"\']*["\'][^>]*>\s*'
    r'Generated by Market Lense\.\s*File ID:\s*<code>[^<]+</code>\.\s*</footer>',
    re.IGNORECASE | re.DOTALL,
)
...
def strip_publication_internal_metadata(html_text: str) -> str:
    """Remove operator-only identifiers before public WordPress publication."""
    without_scripts = _INTERNAL_PUBLICATION_SCRIPT_RX.sub("", html_text)
    without_markers = _HIDDEN_FILE_ID_MARKER_RX.sub("", without_scripts)
    without_footers = _INTERNAL_FILE_ID_FOOTER_RX.sub("", without_markers)
    return _PUBLICATION_FILE_ID_TEXT_RX.sub("", without_footers)
