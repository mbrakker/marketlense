...
- Unified LLM service: `src/services/llm_service.py` is the single public OpenAI, OpenRouter, generic model-call policy, and OpenAI vector-store provider boundary. Semantic implementations live under `src/services/_llm_service/`. Cost accounting is owned by `src/services/openai_accounting_service.py`: SQLite `llm_usage_events` is the canonical source, compatibility JSONL projections validate paired JSONL/daily checkpoints before extension, append newly projected rows under a durable generation lease, and retain immutable incremental segments beside the compatibility export. Pre-call daily-spend policy adds the exact calculated median cost for the matching provider/task/action/model/prompt namespace to canonical UTC spend, then warns at `$3`, pauses at `$5`, and hard-stops at `$6` without overrides; no-match calls are explicitly logged as cold starts. `python scripts/quality/reconcile_llm_usage_ledger.py --db <sqlite> --ledger <jsonl> --daily <json> --output-json <artifact>` writes a structured reconciliation artifact without reading credentials; credentials remain only in `.env`.
...
- `python scripts/ci/check_pdf_candidate_benchmark.py --baseline docs/quality/pdf_candidate_extraction_benchmark_baseline.json --output-json out/pdf_candidate_benchmark.json --fail-on-runtime-regression` (strict local/release PDF candidate benchmark gate; fails on candidate signature/count/degraded-page drift and on documented runtime failure thresholds when existing benchmark PDFs under `cache/benchmark_20260604/` are present)
...
- PDF candidate performance/equivalence baseline: `docs/quality/pdf_candidate_extraction_benchmark_baseline.json` references existing generated dense PDFs in `cache/benchmark_20260604/`. CI runs this gate with `--allow-missing-assets` because those PDFs are local/generated artifacts, but release or local verification should omit that flag so missing assets fail and add `--fail-on-runtime-regression` when runtime failures must block release. Refresh the baseline only after an approved candidate-output change by running `python scripts/quality/pdf_candidate_benchmark.py --baseline docs/quality/pdf_candidate_extraction_benchmark_baseline.json --output-json out/pdf_candidate_benchmark_refresh.json --write-current-baseline docs/quality/pdf_candidate_extraction_benchmark_baseline.json --iterations 2`; review candidate signatures, counts, degraded-page counts, and runtime deltas before committing.
