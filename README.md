...
When an explicit `--delivery-email` is supplied, static email-gate detection no longer stops at `email_required`; the planned browser email-form route is allowed to submit the form and verify `email_requested`. Without an explicit delivery email, static email-gate detection still fails closed as `email_required` before browser spend.
...
`poll-mail-report` is routed through `src/orchestrators/mail_report_acquisition_orchestrator.py`. It loads mailbox settings from `mailbox_acquisition` in `src/config/app.yaml` and mailbox credentials from `.env`, searches the mailbox with source URL, report title, publisher, and delivery-email context, selects only links with report/download/document evidence, and then calls `run_report_download` for the selected URL. Pass `--requested-after-utc` with the UTC time of the form submission when re-testing the same publisher/report so older matching delivery emails are ignored. If the report email has not arrived within the configured polling window, the command raises retryable `mail_report_not_arrived_yet`; this is expected for publishers whose delivery queue takes several minutes.
