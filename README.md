...
- Mailbox-backed gated-report acquisition: `src/orchestrators/mail_report_acquisition_orchestrator.py` polls a configured mailbox after verified email-requested report delivery, uses deterministic mail-link selection, and passes the selected report URL back through the existing report-download workflow. The mailbox service supports IMAP as the local default plus Gmail OAuth when explicitly configured, and delayed delivery exits as retryable `mail_report_not_arrived_yet` instead of a hard failure.
- Mailbox acquisition hardening: browser prompts now apply an explicit delivery mailbox to every email-labeled identity field, not just `work_email`; remembered email-gate URLs refresh email query values before reuse; strong thank-you/inbox confirmation text is classified as verified `email_requested`; worker response artifacts and structured logs redact identity values plus encoded sensitive URL query parameters. BigCommerce has host-scoped identity overrides for its live annual-revenue and company fields so fresh form submissions use values accepted by the current publisher form.
