...
Retry behavior has one owner: orchestrators use `src/orchestrators/retry_orchestrator.py` for bounded workflow retries and backoff. `src/services/llm_service.py` performs exactly one provider attempt, disables OpenAI/OpenRouter SDK retries, retains circuit breaking and optional scope-level rate limiting, and immediately propagates typed failures. Nonzero `ingest.llm` retry/backoff configuration is rejected.
