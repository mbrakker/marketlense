...
Config editing is also split by concern behind the same canonical boundary: `src/services/config_service.py` remains the public service for `app.yaml` reads/writes and browser-download identity upserts, while `src/services/_config_app_document.py` owns app-document editor I/O and `src/services/_config_identity.py` owns browser identity normalization plus upsert planning. The settings UI still calls the canonical service boundary, and structured asset editing remains on `src/services/config_asset_service.py`.
