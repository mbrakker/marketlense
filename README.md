...
- Report-download archival hardening now uses short atomic temp names for long Windows artifact paths and scopes source-record and Drive-upload idempotency by checksum, so repeat browser captures with changed generated PDFs or same-named screenshots do not crash with checksum collisions.
