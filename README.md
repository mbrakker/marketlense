...
- `python scripts/quality/release_evidence_manifest.py --release-id <release-id> --artifact coverage=coverage.xml --expected-schema coverage=<coverage-version> --artifact mutation=mutation_results.json --expected-schema mutation=1.0 --artifact health_scorecard=out/run_health_scorecard.json --expected-schema health_scorecard=1.0 --output-json out/release_evidence_manifest.json` (release evidence bundle manifest; validates retained artifact presence/schema versions, records commit SHA, command arguments, artifact hashes/paths, timestamps, and pass/fail status, and exits nonzero for missing, invalid, schema-mismatched, or failed gate evidence)
