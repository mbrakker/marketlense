...
- Cost and token tracking: `analysis.cost_ledger_path` (`COST_LEDGER_PATH`, default `./out/cost-ledger.jsonl`), `cost.daily_path` (default `./out/cost-daily.json`), `cost.usage_db_path` (`LLM_USAGE_DB_PATH`, default `./state/llm_usage.sqlite`), and `cost.pricing_path` (default `./src/config/llm-costs.yaml`) configure one accounting store. SQLite `llm_usage_events` is canonical; its deterministic event key is unique, excludes unreliable provider request IDs, and includes a call ordinal so retries replay once while distinct calls remain separate. Every row retains provider-call, parse, schema-validation, error-stage, and terminal-error-code outcomes; usage is recorded before output validation and finalized in place. JSONL/daily files remain reproducible compatibility exports, and `llm_usage_ledger_service.reconcile_usage_export` compares event count, input/output/cached-token totals, and estimated cost with an exact `1e-9` cost tolerance. Browser-use uses the same configured paths through a bounded nonblocking callback queue and flushes on shutdown within `browser_download.accounting_flush_timeout_seconds` (default `5`).
