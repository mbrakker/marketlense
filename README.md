...
Enterprise PDF ingestion and analysis pipeline that converts Google Drive reports into MarketBearing executive report pages using an LLM and extraction heuristics.

---

## Executive Summary

Market Lense ingests PDFs from a configured Google Drive folder, extracts text and structured visual candidates (tables/charts), calls an LLM for a strict JSON analysis, ranks and crops key visuals, renders a responsive MarketBearing executive report page, and can publish the page to WordPress. The system is organized around a strict architecture with contracts, services, generators, and orchestrators to ensure reliability, observability, and testability.

Key traits:

- Contract-first data model for all external I/O boundaries.
- Service isolation for all external systems and file I/O.
- Dependency security baseline: the application and vendored browser-use manifests require `pypdf 6.13.3`, `aiohttp 3.14.1`, and `pydantic-settings 2.14.2`, closing the current PDF resource-exhaustion, aiohttp request-handling, and nested secrets-directory advisories.
- Non-long-module audit repair: recent-theme artifact loading now enters file I/O through a typed `read_text_files` service contract, OCR and validation cache failures preserve orchestrator-owned retry semantics, analytics projection writes batch multirow upserts, current golden `artifacts.json` fixtures carry required cover semantics plus compact TLDR card copy, and browser-use/root dependency pins are aligned so `pip check` is clean.
- Browser-report runtime isolation: `src/services/_browser_report_download/browser.py` remains the private coordinator behind the canonical browser-download service, while terminal-state stabilization, terminal asset/evidence capture, bounded timeout recovery, worker transport, and browser session lifecycle mechanics live under `src/services/_browser_report_download/_browser_runtime/`. Terminal asset capture now keeps `terminal_assets.py` as a compatibility surface over focused artifact, capture, network, and page-state owners.
- Browser-report HTTP isolation: `src/services/_browser_report_download/http.py` remains the private compatibility surface, while PDF transfer, page-PDF probing, gate probing, onsite capture, and shared HTTP/HTML evidence live in `_http/` capability modules behind the canonical browser-report service boundary. Visible HTML evidence sanitization excludes `script` and `style` blocks even when their closing tags contain whitespace or trailing malformed tag content, so hidden confirmation or CTA tokens cannot affect terminal classification. Direct PDF fetches first use browser-compatible headers, then retry once with PDF-only headers when the publisher stalls browser-style user agents; the fallback is logged as `browser_report_download_pdf_fetch_header_fallback`.
...
- Mailbox-backed gated-report acquisition: `src/orchestrators/mail_report_acquisition_orchestrator.py` acquires delayed email-delivered reports through the existing report-download workflow, matches deliveries to a specific source URL/report/publisher/email request, materializes PDF attachments and PDF files inside ZIP attachments before link follow-up, records seen provider message IDs so retries do not rescan already-checked mail, persists request-scoped candidate rejection evidence, and promotes successful mailbox outcomes into publisher route memory for future pre-browser planning.
- Event-driven email acquisition handoff: report-download outcomes classified as `email_requested` or `email_required` now create durable `mail_delivery_requests` in the state database and emit workflow-control observations for the deferred `mail_acquisition` workflow. `workflow_control_orchestrator.run_due_mail_delivery_requests(...)` dispatches due mailbox requests without an operator CLI, carries seen-message state forward, and marks requests succeeded, pending, or failed through the state service. Browser runs preflight the configured mailbox before submitting gated forms when a delivery email is available.
- Mailbox acquisition hardening: browser prompts now apply an explicit delivery mailbox to every email-labeled identity field, not just `work_email`; `src/config/browser_download_identity.yaml` is the single source for reusable form completion values, delivery-email pool entries, host-scoped overrides, and typed consent policy. Required privacy/terms/report-delivery agreements are allowed by policy, while optional marketing/newsletter consent defaults to off. Remembered email-gate URLs refresh email query values before reuse, strong thank-you/inbox confirmation text is classified as verified `email_requested`, and worker response artifacts plus structured logs redact identity values and encoded sensitive URL query parameters.
...
- Pipeline preflight gate: `report_pipeline_orchestrator` now runs a typed preflight before model clients or expensive work are started. `src/orchestrators/pipeline_preflight_orchestrator.py` checks local writability, required prompt namespaces, LLM settings, and optional bounded Drive/WordPress/browser readiness; it returns `PreflightReport` blockers, warnings, auto-fixes, and exact next actions. Drive OAuth refreshes performed by the existing Drive service are surfaced as `auto_fixed` instead of being reimplemented.
- Workflow-aware control plane: `src/orchestrators/workflow_control_orchestrator.py` exposes typed preflight profiles, DAG/state-machine contracts, retry policy lookup, operational-memory recommendations, and adaptive concurrency decisions for report generation, publisher inventory, report download, cross-report analysis, publishing, UI replay, WordPress sync, and browser acquisition. The operator-tunable YAML map lives under `workflow_control` in `src/config/app.yaml`, while `config_service.load_workflow_control_settings(...)` keeps YAML access inside the service layer.
- Plan-first operator safety: CLI and UI workflow-control resolution now attach one typed, side-effect-free `PipelineExecutionPlan` before dispatch. It records the selected workflow/profile, ordered and blocked steps, credential boundaries, checkpoints/expected artifacts, planned side effects, and a scoped idempotency key, so the planned execution can be inspected without making an external change.
- Workflow-control execution wiring: CLI ingest/publish and UI-launched background runs now resolve a typed `RunIntent` before side effects, log the selected workflow, preflight profile, retry policy, side-effect plan, and resume stage, and persist compact workflow-control observations back to the state database for future operational-memory and adaptive-concurrency planning.
- Autonomous gates and audit records: workflow control now converts preflight next actions into typed safe remediation artifacts, applies policy-driven publish decisions with validation/confidence gates, evaluates deterministic pre-LLM data-quality gates before expensive model work, resolves adaptive concurrency across model, PDF, browser, Drive, and WordPress resources, and emits replayable model-call audit records containing prompt hashes, redaction hashes, model parameters, schema metadata, response IDs, token counts, cache decisions, and validation status.
- Automatic checkpoint resume: `report_pipeline_orchestrator.run_report_pipeline(..., auto_resume_from_latest_safe=True)` now passes `latest_safe` when no explicit `resume_from_stage` is supplied, so reruns can automatically resume from the newest valid report-generation checkpoint and let the existing checkpoint validator reject corrupt, stale, or missing artifacts.
- Canonical artifact lineage: report checkpoints now persist immutable SHA-256 content-addressed records and direct dependency edges in `reports.sqlite`; reuse verifies schema/processing/prompt/model/validation compatibility and current storage content, not just a file path. `backfill-artifact-lineage --dry-run` scans retained checkpoints without mutation, while `--apply` is bounded and idempotent. Source, prompt, template, crop, and validator changes can invalidate only their dependent graph; render-only changes leave extracted text and analysis reusable.
- Selective regeneration now requests the smallest safe restart from the change kind and independently verifies the retained checkpoint lineage before reusing it. A missing or invalid lineage record is a typed `report_pipeline_checkpoint_lineage_missing`/`report_pipeline_checkpoint_lineage_not_reusable` failure, never a caller-controlled reuse decision.
- Prompt, resume, acquisition, and ingest hardening on 2026-07-08 added prompt-service includes, schema snippets generated from JSON schemas, route-family browser prompt namespaces, strategic insight scoring fields (`decision_relevance_score`, `metric_strength_score`, `novelty_score`, `coverage_role`, `so_what`, `now_what`, `report_type_lens`), critique-first regeneration prompt partials, severity-aware regeneration planning, fast-first vector-store polling, md5-keyed vector-store reuse, latest-safe ingest wiring, Drive/cache prefetch before report workers, artifact-level browser acquisition cache with MD5/SHA-256 validation, deterministic pre-LLM form autofill for confident email gates, and parallel table/chart candidate ranking under existing worker caps. Verification covered prompt dry-run and fixture-corpus regression, focused tests, `ruff` on changed Python files, and a live existing IAS checkpoint resume returning `processed` in 0.005s with zero model/API calls.
- Artifact, validation, Drive, and browser acquisition hardening on 2026-07-10 added migration-safe `topics_covered`, `key_figures`, and `chart_insight_cards` artifacts; deterministic anti-generic artifact-quality validation with source-quote and technical-term allowances; validation modes for `full`, `inline_deterministic`, and `deferred_grounding`; cursor-first Drive ingest with explicit `--rescan`; batch processed-state metadata reuse; latest-safe fresh fallback when no valid checkpoint exists; invalid cached-PDF redownload with EOF hard-stop before report-pipeline work; canary route/private-API promotion; deterministic route-playbook execution; stricter publisher route policy before browser escalation; narrower browser preflight with reusable cookie/local-storage state; metadata-only forensics for expected blockers; and direct-PDF route plans that stay PDF-only instead of escalating into browser-use.
- Public advisory and crop QA hardening on 2026-07-10 exposes `executive_advisory`, strongest `metric_spine` proof points, readable claim-support labels, and `so_what`/`now_what` in report HTML while redacting canonical claim/evidence IDs. Rendered pages carry `editorial_contract_version=public-report-editorial-v1`, and publish-time editorial checks block generic phrasing or internal-reference leakage with stable rule IDs before WordPress side effects. Claim-support validation now grades evidence support and sends strong claims backed only by weak evidence to targeted summary regeneration. `CropResponse` includes typed per-candidate outcomes, `publication_strict` is the canonical user-facing crop path, and figure assets/candidate packs carry accepted status, QA score, defect labels, sidecar path, quality profile, and rejection reason.
- Public quality gates on 2026-07-11 extend the publish path with low-confidence model-backed crop QA escalation from retained `publication_strict` `.qa.json` sidecars, a public report intelligence panel for topics covered, key figures, and chart insight cards, broader editorial warning rules, retained public-advisory render benchmarking, publish remediation targets for artifact-quality warnings, autonomous supervisor planning, and production run-health gate decisions. Published report meta now carries `ml_public_intelligence`, so the WordPress report archive can filter reports with public intelligence modules; existing report-card updates recompute the flag from the retained report body and preserve eligibility when the public intelligence panel is present. Browser identity learning can promote safe required-select values from live gated-form evidence into host-scoped overrides while refusing sensitive identity fields.
- Live verification for the 2026-07-11 quality gates used existing retained artifacts instead of synthetic fixtures: 73 retained low-confidence `publication_strict` sidecars were discovered, one real `gpt-5-mini` crop QA escalation ran against a retained image sidecar and returned `accept` with provider request ID `resp_03b3d64d908382f0006a51cf45a264819d875daa9635979961`, and a retained Activate report artifact was regenerated with 7 topics, 4 key figures, 4 chart insight cards, 4 metric-spine entries, and 17 claim-ledger entries. The retained advisory benchmark reported advisory coverage `1.0`, internal ID leaks `0`, remediation targets `0`, and the Playwright desktop/mobile screenshots under `out/live_feature_verification/` confirmed the public intelligence panel renders without overlap.
- Executable architecture policy is now centralized in `docs/quality/architecture_policy.yaml`. CI quality scripts consume it for role import/I/O rules, service-boundary ownership, coverage and mutation thresholds, mypy baseline expiry, Ruff lint policy, repository hygiene, and movement-evidence requirements. The root `pyproject.toml` is the canonical Python tool manifest, runtime/dev dependencies are pinned through `requirements.lock`, and CI installs from that lockfile. Browser-download routes can now define per-route-family agent budgets under `browser_download.route_budgets`, capped by the global `timeout_seconds` and `max_steps`, so simple routes use tighter browser-use envelopes without silently increasing spend.
- Centralized retry policy resolution: workflows can pass `WorkflowControlSettings` into `run_report_pipeline(...)`; the report pipeline resolves `report_generation.report_pipeline.v1`, logs the policy ID/version on start and retry decisions, and uses the shared retry orchestrator policy object instead of hard-coded step settings.
...
- Retry-decision telemetry: `scripts/quality/retry_decision_telemetry.py` and the run health scorecard aggregate structured retry decision logs by step, error code, publisher/workflow when present, action, reason, attempts, delay, final outcomes, successful-after-retry rate, retry-exhaustion rate, deferred/user-action-required counts, and estimated avoided or wasted calls.
- Structured logging with run/task/span identifiers plus end-to-end trace IDs and nested span metadata.
- Built-in validation: semantic checks plus LLM grounding with persisted reports and publish-time policy controls. Validation requests can run in `inline_deterministic` mode for schema/reference/completeness/artifact-quality checks without constructing a model client, or in `deferred_grounding` mode for grounding-only follow-up before publish/high-value paths.
- Evidence quality policy: claim-support validation recognizes direct metrics, direct quotes, explicit findings/recommendations/risks, chart readouts, source-backed spans, section summaries, and weak paraphrases. Strong numeric or absolute claims backed only by weak evidence emit `claim_support` errors with `repair_target="summary"` so regeneration stays targeted instead of rerunning unrelated artifact families.
...
- Confidence-scored family outputs: every evidence pack now persists a typed `family_status` record with `status`, `confidence_score`, `policy_action`, and `reason`, and generated artifact families do the same at the top level of `artifacts.json`. Low-confidence summary/insights families abstain into explicit validation regeneration targets instead of shipping weak output; quote families require a real verbatim source, and quote-only abstentions now publish as warning-level omitted-quote notices instead of forcing fake quotation text. Soft editorial families such as `expert_comment` and `linkedin_post` can also abstain with warning-level omission notices that render transparently in the HTML report page.
...
- Source-PDF validation quarantine: if Drive returns the same artifact after bounded redownloads and it still has no PDF EOF marker, ingest records the `(file_id, md5)` in processed state with `pdf_download_missing_eof` and `text_validation_status=fail`. Later limited ingests skip that exact corrupt artifact instead of repeatedly consuming the batch slot; if a newly selected candidate fails before report generation, the same limited run continues to the next fresh candidate until the requested processed count is reached or Drive has no more candidates. A revised Drive file with a new MD5 remains eligible.
...
- Durable report-generation checkpoints: source, selection, analysis, and render stages persist versioned checkpoints under `out/.checkpoints/report_generation/<file_id>/` with artifact references, artifact integrity metadata, and restart payloads. The report pipeline can restart from `source_prepared`, `selection_complete`, `analysis_complete`, or `render_complete`; `latest_safe` automatically selects the newest checkpoint whose persisted artifacts still validate.
- Deterministic PDF artifact fingerprints: preview PNGs, crop-refine page renders, and saved crop PNGs now reuse existing artifacts when a page-content fingerprint plus artifact identity, settings, and PyMuPDF/cache-version fingerprints still match. `publication_strict` crop cache hits additionally require an accepted final QA sidecar; rejected diagnostics return a typed non-publishable outcome rather than an image path, and missing/invalid diagnostics regenerate the crop. The sidecar metadata lives next to the artifact, reruns can keep unaffected page/figure outputs warm even when another page in the document changes, and cache-hit/store logs emit the key, source artifact, and validity reason.
- PDF image artifact filenames retain their normal report-based form when bounded; oversized crop, preview, figure, and thumbnail names use deterministic compact stems plus hashes so long report slugs remain writable on Windows without changing the report directory or risking filename collisions.
- PDF crop isolation: `src/services/_pdf/crop.py` remains the internal compatibility facade behind `pdf_service`, while image trimming/composition, crop geometry guards, table-continuation stitching, crop-region artifact writes, crop-refine rendering, and preview rendering live in focused private `_crop/` modules without changing artifact fingerprint semantics, render settings, or crop ordering.
- Drive listing efficiency: `src/services/drive_service.py` now streams Drive file pages incrementally instead of materializing full page sets up front, caches recursive folder-scope expansion with TTL-based invalidation, and bounds thread-scoped Drive client reuse with expiring max-size caches so long-lived runs do not accumulate stale clients. Drive client construction and OAuth token refresh run outside the client-cache lock, so refresh-triggered cache invalidation cannot deadlock an ingest run; Drive clients also use static discovery consistently for list/download/preflight paths. Ingest defaults to cursor-first listing even with `--limit`; operators use `--rescan` or forced report-card runs when they intentionally need a full folder scan.
- Drive/UI/report-source execution isolation: `src/services/drive_service.py`, `src/ui/app_pages/publisher_operations.py`, `src/orchestrators/ui_run_execution_orchestrator.py`, and `src/generators/report_source_generator.py` now remain compatibility facades over focused private capability modules. Public imports, Drive patch seams, UI-run replay fingerprints, prompt/model call counts, and report-source cache behavior are preserved.
- Runtime long-module isolation: `src/orchestrators/report_generation_orchestrator.py`, `src/services/wordpress_service.py`, `src/services/render_service.py`, `src/services/_pdf/_visual_candidates/extraction.py`, and `src/services/_browser_report_download/_artifact/classification.py` now remain compatibility facades over focused private owner modules. Report checkpoints/resume/workflow, WordPress transport/posts/taxonomy, render normalization/view/workflow, visual extraction context/sequential/workflow, and artifact classification evidence/routes/workflow are split without changing external calls, prompt/model usage, route ordering, cache keys, artifact paths, or public imports.
- Requested runtime long-module isolation: `src/contracts/cross_report_analysis.py`, `src/services/_pdf/_table_heuristics/{regions,screening}.py`, `src/services/_browser_report_download/_browser_runtime/session_lifecycle.py`, and `src/services/_report_store_service/download_routes.py` now remain compatibility facades over semantic private owner modules. Cross-report contract families, table screening/region mechanics, browser session history/partial-history/cleanup/shutdown, and report-store private-API/route lookup/route recording are split without changing public imports, route ordering, external calls, prompt/model usage, cache keys, artifact paths, or cost behavior.
...
- Capability observability map: `scripts/quality/capability_maps.py` writes `docs/quality/capability_maps.json`, listing canonical service boundaries, workflow owners, artifact schemas, state tables, failure runbooks, smoke suites, and idempotency scopes for every tracked external side effect.
...
- Long-file service and generator internals retain their canonical import paths while using semantic private owners: SQLite migrations are grouped by database schema, publisher candidate quality by classification/evaluation/workflow, browser-report CDP by transport/session/dialog/operation capability, and publisher inventory acquisition by fetch and browser-traversal capability. The split is movement-only and preserves call ordering, network/model usage, logging, and cost behavior.
...
- HTML report quality: rendered HTML now uses semantic sections (`header/main/section`), SEO metadata, responsive grids, MarketBearing brand motifs, a compact executive hero, report-value scoring, TLDR/executive summary, source-backed findings, quote cards, optional visual-candidate carousel, expert commentary, collapsible LinkedIn copy, and long-text chunking for generated prose.
...
- HTML report pages now keep report-derived metadata when the reports DB has gaps, show compact report identity in the hero, emit an explicit missing-source note when no source URL was extracted, expose a relative local PDF download link when available, and keep generated expert/social copy as first-class report output sections.
...
- OpenAI Responses compatibility: `seed` is retained in contracts/cache identity but omitted before current `responses.create` SDK calls, and GPT-5-only unsupported params such as `temperature` are also omitted before image and vector-backed Responses API calls. Unknown model/parameter mismatches fail once as typed non-retryable bad requests instead of issuing an unowned second provider call.
- Unified LLM service: `src/services/llm_service.py` is the single public OpenAI, OpenRouter, generic model-call policy, and OpenAI vector-store provider boundary. Semantic implementations live under `src/services/_llm_service/`. Cost accounting is owned by `src/services/openai_accounting_service.py`: SQLite `llm_usage_events` is canonical; every public model-call finalization materializes its compatibility JSONL/daily projection through a fenced generation, immutable segment, and rolling digest without rereading historical JSONL. Pre-call OpenAI/OpenRouter policy adds the exact matched median plus atomically reserved in-flight spend, releases the reservation on canonical recording, then warns at `$3`, pauses at `$5`, and hard-stops at `$6`; cold starts are explicit. `python scripts/quality/reconcile_llm_usage_ledger.py --db <sqlite> --ledger <jsonl> --daily <json> --output-json <artifact>` performs a full canonical comparison and repairable reconciliation without reading credentials; credentials remain only in `.env`.
- Shared LLM call policy: the private LLM client and policy capabilities enforce one provider attempt, circuit breaking, and optional scope-level rate limiting. Report-pipeline evidence/artifact clients and default generator LLM clients use generic builders from the canonical boundary instead of provider-named construction helpers.
...
- Canonical LLM call path: production generators, report-pipeline orchestration, ranking/crop-refine, OCR/image-call dependency wiring, vector-store provider adapters, and OpenRouter browser-model construction enter model/provider calls through `src/services/llm_service.py`. Browser lifecycle and agent execution remain in the browser-download service. The shared boundary logs provider/cache/budget-policy context together with explicit orchestrator retry ownership, rate-limit, circuit-breaker, and sanitized OpenRouter client-construction events.
- Browser acquisition is governed by `browser_download.run_budget` in `src/config/app.yaml`. Before each governed browser attempt or Drive archival write, the download orchestrator reads the canonical `llm_usage.sqlite` ledger across the exact run, UTC day, and publisher scopes and applies the most conservative metric value. The default profile caps browser launches, PDFs, and Drive writes at 30 and uses the existing daily spend hard stop; completed PDF downloads, actual browser launches, and Drive writes are recorded idempotently in the shared ledger rather than a second accounting database. Direct HTTP PDF routes count toward the PDF cap but not the browser-launch cap.
- Model-client ownership: orchestrators and service-factory wiring build scoped model clients, then inject them into model-backed generators. Generators render prompts, validate contracts, and call the injected client; they do not construct provider-policy clients from settings.
...
- Refusal-aware error handling: OpenAI content-filter/refusal/policy, auth, bad-request, and other permanent provider errors are normalized into typed non-retryable `AppError` codes. The LLM service logs the concrete retry decision and propagates every failure after one provider attempt so only the calling orchestrator can retry.
...
- Browser email-form prompt hardening: required agreement/privacy/terms checkbox fields now treat true-like configured identity values as explicit permission to check only the matching required checkbox, optional marketing/newsletter opt-ins remain unchecked unless explicitly configured, and same-origin modal iframe forms can be opened directly when visible iframe controls are not indexed by the browser runtime.
- Browser identity enum handling: identity fields in `src/config/browser_download_identity.yaml` may declare `option_aliases` for safe visible select/enum labels that represent the configured value, allowing dropdown recovery to match publisher wording without inventing answers.
- Browser identity fallback handling: when a required enum/select/dropdown has no configured match, the browser prompt now chooses the first visible enabled non-placeholder option and records the selected label; safe generic text fallbacks cover company size, revenue, business type, country/location, role, industry, department, and priority fields.
...
- Browser-download route validation now avoids several live-run stall amplifiers found in the April 21, 2026 random report probe. Tracker detection uses exact host/path tokens so ordinary publisher hosts are not misrouted, tracker-like hosts with report-detail slugs and no explicit redirect target now use the direct candidate URL instead of idling on the source listing page, weak email-route memory only influences fallback when it contains actionable form/download evidence, `/reports/<detail>` URLs are no longer treated as listing hubs purely because the slug is short, generic whitepaper/ebook/download/register paths route directly to email-form planning instead of wasting a browser-PDF attempt, and singular `/insight/<slug>` plus guide/playbook/research-style URLs route to onsite capture. The service now probes report-detail candidate HTML for direct onsite capture before browser-use when the fetched page is a real article, can also recover long full-report pages that were initially planned as email forms when no lead-capture form remains in the fetched HTML, materializes browser-use temp PDFs and missing onsite captures into the managed download directory before temp cleanup, fetches observed terminal document URLs including relative `.pdf` links before accepting a blocked form result, repeats that PDF completion pass after terminal HTML recovery when the initial browser payload was too thin or too large to inline, ignores worker metadata JSON as candidate downloads, and uses token-aware non-report marker checks so words such as `self-expression` do not block report salvage.
- CAPTCHA-protected email routes use `browser_download.captcha_handoff`: when enabled, a remembered or newly detected `blocked_captcha` route opens one headed browser handoff for the configured operator window and navigates that visible page to the report URL before waiting. The operator may complete the CAPTCHA manually; automation must not solve or bypass it. If the window expires, the route is recorded as `blocked_captcha` with manual-handoff timeout evidence and the workflow proceeds as a failed report acquisition.
- Publisher route memory now expires after `browser_download.route_memory_ttl_seconds` (30 days by default). Expired, missing-timestamp, or clock-invalid records remain retained for review but cannot steer an unattended attempt; the planner logs the expiry and returns to normal discovery rather than suppressing alternate routes.
...
- Browser-download runs now include a bounded browser preflight before full browser-use agent launch for eligible browser route families. The preflight lives inside `src/services/_browser_report_download/preflight.py`, uses a short Marketlense-owned browser-use session to open the execution URL, collect page info, run a small rendered-DOM JS probe, and drain resource-event URLs, then either downloads a rendered direct PDF as `browser_preflight_js_pdf_probe` or logs escalation evidence for the full agent. Email-form and listing-hub preflight now require positive static/candidate evidence before browser startup, and successful preflights persist reusable state such as cookies, local-storage keys, current URL, and downloaded-candidate context for escalation. Preflight metrics log avoided agent calls, duration, candidate counts, escalation reason, reuse-state availability, and a per-run false-negative sample once the full agent outcome is known.
- Browser-download route playbooks now live as independent YAML files under `src/playbooks/browser_routes/`, with the format documented beside them. The browser-download service loads fresh playbooks before browser-use prompt rendering, selects matches by host/path marker plus route family, logs selected `playbook_id@version` values, and falls back to normal discovery or raises `browser_route_playbook_stale` according to `browser_download.route_playbook_stale_policy`. Normal playbooks can carry deterministic open/click/fill/select/verify step selectors for direct execution before full browser-use, with drift reported as typed step evidence. Validated successful browser route and private-API candidates can run in `canary` promotion mode, which logs review-required promotion output without writing playbook files.
...
- CLI command wiring keeps `src.cli` as the canonical `python -m src.cli` entrypoint while private command-family owners now live under `src/_cli/` for app bootstrap, report pipeline, browser, publisher, cross-report, private-API, trace, admin, and UI-run commands. The facade intentionally re-exports existing symbols so imports and public patch points remain stable.
...
- Figure quality gate: candidate visuals now pass deterministic prefilters, kind-split ranking, adaptive GPT crop refinement, and `publication_strict` final cropping before HTML render. The publication crop path routes tables/charts through their strict geometry modes, renders final selected gallery crops with the explicit `rank.final_crop_dpi` profile, snaps high-confidence table outer rules, applies content-aware trim, runs final PNG QA with table-boundary, chart-completeness, and visual-card-boundary detectors, and writes `.qa.json` diagnostics sidecars before accepting public crops. Crop responses also return typed per-candidate outcomes keyed by candidate ID, so rejected first crops cannot shift later accepted crops onto the wrong figure; accepted figure assets and candidate packs persist QA score, defects, sidecar path, quality profile, and rejection reason for telemetry/review. The detailed extraction, crop, and fallback heuristics live in [Technical Design Notes](#technical-design-notes).
- Candidate extraction split: `pdf_service.collect_candidates` remains the canonical PDF-candidate service boundary. `table_heuristics.py` is the stable internal compatibility surface over `_table_heuristics/{policy,models,layout,regions,screening}.py`; `regions.py` now delegates ranked detection and bbox composition to `_table_heuristics/_regions/`, while `screening.py` delegates metrics, rejection policy, and deduplication to `_table_heuristics/_screening/`. `visual_heuristics.py` is the stable facade over `_visual_heuristics/{chart_layout,panel_text,panel_geometry,panel_detection,collectors}.py`; `panel_detection.py` now delegates shadowing and candidate assembly to `_visual_heuristics/_panel_detection/`; and `visual_candidates.py` is the stable facade over `_visual_candidates/{raster,screening,extraction}.py`. Heuristic rationale and known limits are grouped in [Technical Design Notes](#technical-design-notes).
- Table candidate dedupe uses a page-local spatial lookup owned by `_table_heuristics/_screening/deduplication.py` and re-exported through `_table_heuristics/screening.py` for both pdfplumber candidates and the final table merge in `table_candidates.py`, so dense reports avoid repeated full scans while preserving the existing IOU, containment, ranked-overlap, and inner-lattice preference rules.
...
- Publish file ID resolution is DB-first: publish/publish-queue share the same HTML-path canonicalization and reports-metadata (`html_path -> file_id`) lookup helper, then fall back to HTML parsing only when mapping is unavailable. Auto-discovered publish runs order DB-backed report HTML by reports metadata `updated_at` before applying `--limit`, so a fresh ingest batch is not displaced by older unpublished files; targeted publish runs can still pass explicit HTML paths and preserve that caller-provided order.
- Configured WordPress publication: new reports use the configured post status, which defaults to `publish`. Use `python -m src.cli publish-wp --draft` to create review-only drafts for a run. Existing report-card metadata updates retain their existing post status.
- Pipeline-owned WordPress intelligence: `python -m src.cli sync-wordpress-intelligence` reads the authenticated plugin's raw approved reports, briefings, signals, publishers, and topics; the Python generator computes the current projection; and the protected `marketlense/v1/intelligence-projection` route stores it. Homepage metrics, freshness, trends, strategic themes, and publisher authority render only this validated projection; a missing or invalid projection is deliberately neutral rather than inferred from WordPress counts.
...
- Generated report documents publish body-only report markup. WordPress owns the public site header and footer, disables automatic paragraph insertion for report post types, scopes the legacy ingest design away from `.report-document`, and then applies an unlayered scoped copy of `templates/report.css.j2`. Rendering contract tests compare the scoped declaration set with the generator stylesheet so template design changes cannot silently drift from published reports.
...
- `docs/architecture/analytics-publish-decomposition-review.md`: movement audit and boundary review for analytics persistence, reports schema migrations, and publication orchestration.
- `docs/architecture/long-file-semantic-decomposition-review.md`: movement audit and boundary review for SQLite migrations, publisher candidate quality, browser-report CDP, and publisher inventory fetch/browser traversal.
- `docs/architecture/requested-runtime-decomposition-review.md`: movement audit and boundary review for cross-report contracts, PDF table heuristic internals, browser session lifecycle, and report-store download-route persistence.
- `docs/architecture/requested-projection-pdf-publisher-decomposition-review.md`: movement audit and boundary review for analytics projection, crop refinement, PDF panel detection, and publisher discovery activity.
...
- `docs/architecture/cli-decomposition-review.md`: movement audit and boundary review for the canonical CLI facade and private command-family modules.
...
- **Contract family facades**: Large contract surfaces keep one public module boundary, while semantic dataclass families can live under same-name internal subfolders. Current examples include `src/contracts/publisher_inventory.py` with `src/contracts/_publisher_inventory/*`, `src/contracts/browser_download.py` with `src/contracts/_browser_download/*`, `src/contracts/report_store.py` with `src/contracts/_report_store/*`, and `src/contracts/cross_report_analysis.py` with `src/contracts/_cross_report_analysis/*`. Shared bounded-context vocabulary belongs to the internal package owner instead of a one-off `common.py` module.
- **Implementation family facades**: Large first-party service, generator, UI, and orchestrator surfaces keep their canonical public import path as a facade while semantic implementation families live under a same-name internal folder. Current examples include `src/services/config_service.py` over `src/services/_config_service/*`, `src/services/llm_service.py` over `src/services/_llm_service/*`, `src/services/drive_service.py` over `src/services/_drive_service/*`, `src/services/publisher_inventory_service.py` over `src/services/_publisher_inventory_service/*`, `src/generators/artifact_generator.py` over `src/generators/_artifact_generator/*`, `src/generators/report_source_generator.py` over `src/generators/_report_source_generator/*`, `src/orchestrators/report_download_orchestrator.py` over `src/orchestrators/_report_download_orchestrator/*`, `src/orchestrators/ui_run_execution_orchestrator.py` over `src/orchestrators/_ui_run_execution_orchestrator/*`, and `src/ui/app_pages/publisher_operations.py` over `src/ui/app_pages/_publisher_operations/*`.
...
- `src/orchestrators/ingest_orchestrator.py::run_ingest` is the batch entrypoint for Drive listing, ingest locking, worker fanout, cursor updates, batch processed-state skip decisions, explicit rescans, and retention cleanup.
- `src/orchestrators/ingest_file_orchestrator.py::run_ingest_file` is the single-file entrypoint for cache/download/EOF checks, invalid cached-PDF redownload, existing-output skips, state recording, and invoking the canonical report pipeline. If a Drive PDF remains missing its EOF marker after bounded redownload attempts, ingest raises a typed non-retryable source error before entering PDF info extraction or report generation.
...
- `resume_from_stage` supports `source_prepared`, `selection_complete`, `analysis_complete`, `render_complete`, and `latest_safe` through `run_report_pipeline(...)` and `run_report_generation(...)`. Later checkpoints skip more completed work: `selection_complete` reuses the persisted vector-store indexing state, `analysis_complete` re-renders/projects without upstream model stages, and `render_complete` returns the validated persisted `IngestOutcome`.
...
Current public projection scope is intentionally narrower than the discovery vocabulary above. Reports, Topics, Signals, Briefings, Publishers, Sources, Methodology, and internal Data Points have typed projection or workflow contracts. Figures are public only as report-local key figures/exhibits derived from validated report artifacts; standalone Figure directory routes are not yet a public entity contract. Regions and Time Periods are approved metadata/filter values on Reports, Signals, and Briefings, not standalone public entity pages. If standalone Figure, Region, or Time Period routes are introduced later, they require new projection contracts, deterministic empty states, publish/readback tests, and README route documentation before they can be treated as public entities.

Public metadata governance runs before report-card manifests reach publication surfaces. `src/generators/report_card_projection.py::validate_public_metadata_governance` rejects placeholder/default publishers, extracted-field leakage such as `Region: Europe` or `YouGov Year: 2024`, sentence-like region/period values, and missing required public card values with `AppError(code="public_metadata_governance_blocked")`. Blocked entities must be repaired from source-backed artifacts or explicit audited operator overrides; the gate does not invent replacement publisher, region, or period values.

Report pages and report cards should link to related Topics, Signals, Publishers, report-local Figures, and Briefings when those relationships are recoverable. Topic, Signal, Publisher, and Briefing pages should be assembled from approved projections and validated artifacts rather than inferred at render time.
...
First-release non-goals: no metric normalization, unit conversion, or cross-publisher statistical harmonization; no new WordPress plugin or custom post-type dependency; no broad semantic-search product UI beyond local durable claim embeddings; no new deployable worker, microservice, package, or external search service.

The cross-report contract family starts in `src/contracts/cross_report_analysis.py`, which remains the public compatibility surface over focused private owners in `src/contracts/_cross_report_analysis/`, with schema version `1.0`. The bounded-context package owner `src/contracts/_cross_report_analysis/__init__.py` owns shared vocabulary such as publication modes, projection readiness states, content classes, validation status, publish status, and evidence-agreement types. The public facade defines the supported import surface for callers: request, orchestrator request, theme candidate, selected theme, source report candidate, selected source report, projected-data read request/response, evidence reference, signal score and signal-score result, evidence agreement group/result, raw metric reference, generated section, generated analysis result, validation result, persisted analysis artifact, cross-report publish package, publish request summary, publish result summary, orchestrator outcome contracts, and `validate_cross_report_contract`. Required semantic fields are validated by `validate_cross_report_contract`, which raises non-retryable `AppError(code="cross_report_contract_invalid")` when a contract is incomplete, uses an unsupported schema version, or passes `None` for list-typed fields; empty lists remain allowed where the contract marks them optional.
...
First-release non-goals: no metric normalization, unit conversion, or cross-publisher statistical harmonization; no separate WordPress plugin beyond the bundled Market Lense plugin; no broad semantic-search product UI beyond local durable claim embeddings; no new deployable worker, microservice, package, or external search service.
...
- Browser downloads: `OPENAI_API_KEY` is used as the primary browser-use LLM credential, `OPENROUTER_API_KEY` remains the fallback credential, `OPENROUTER_HTTP_REFERER` is optional, `browser_download.model` (`BROWSER_DOWNLOAD_MODEL`, default `gpt-5-mini`) configures the OpenAI primary model, `browser_download.openrouter_model` (`BROWSER_DOWNLOAD_OPENROUTER_MODEL`, default `openai/gpt-5-mini`) configures the fallback model, `browser_download.identity_config_path` (`BROWSER_DOWNLOAD_IDENTITY_CONFIG_PATH`, default `src/config/browser_download_identity.yaml` relative to `app.yaml`), `browser_download.temperature` (`BROWSER_DOWNLOAD_TEMPERATURE`, default `0.0`), `browser_download.timeout_seconds` (`BROWSER_DOWNLOAD_TIMEOUT_SECONDS`, default `180`), `browser_download.max_steps` (`BROWSER_DOWNLOAD_MAX_STEPS`, default `30`), `browser_download.route_budgets.<route_family>.timeout_seconds` / `max_steps` for per-route browser-agent caps, `browser_download.max_tokens` (`BROWSER_DOWNLOAD_MAX_TOKENS`, default `12000`, capped to the browser-use LLM safety limit), `browser_download.output_dir` (`BROWSER_DOWNLOAD_OUTPUT_DIR`, default `./out/browser_downloads`), `browser_download.headed` (`BROWSER_DOWNLOAD_HEADED`, default `false`), `browser_download.captcha_handoff.enabled` (`BROWSER_DOWNLOAD_CAPTCHA_HANDOFF_ENABLED`, default from config), `browser_download.captcha_handoff.timeout_seconds` (`BROWSER_DOWNLOAD_CAPTCHA_HANDOFF_TIMEOUT_SECONDS`, default `120`), `browser_download.drive_upload.enabled` (`BROWSER_DOWNLOAD_DRIVE_UPLOAD_ENABLED`, default `true`), `browser_download.drive_upload.required` (`BROWSER_DOWNLOAD_DRIVE_UPLOAD_REQUIRED`, default `true`), `browser_download.drive_upload.parent_folder_id` (`BROWSER_DOWNLOAD_DRIVE_UPLOAD_PARENT_FOLDER_ID`, falling back to `ingest.gdrive_folder_id` / `GDRIVE_FOLDER_ID`), `browser_download.failure_forensics.enabled` (`BROWSER_DOWNLOAD_FAILURE_FORENSICS_ENABLED`, default `true`), `browser_download.failure_forensics.policy` (`BROWSER_DOWNLOAD_FAILURE_FORENSICS_POLICY`, `copy_artifacts` or `metadata_only`), and `browser_download.retry.*` (`BROWSER_DOWNLOAD_RETRIES`, `BROWSER_DOWNLOAD_BASE_DELAY_SECONDS`, `BROWSER_DOWNLOAD_BACKOFF_STEP_SECONDS`, `BROWSER_DOWNLOAD_JITTER_SECONDS`). The browser-download flow uses the shared `paths.state_db` to persist one remembered route summary per normalized URL, appends newly seen form labels into the identity YAML for later manual completion, preflights required Google Drive archival before expensive acquisition, uploads successful local terminal artifacts to the resolved publisher `google_folder` when Drive archival is enabled, creates and records a publisher-named folder when the publisher row has no folder, and persists failed-attempt forensic packs beside the per-URL download directory when failure forensics are enabled.
- Browser-download session reuse: `browser_download.session_reuse.enabled` (`BROWSER_SESSION_REUSE_ENABLED`, default `false`), `mode` (`BROWSER_SESSION_REUSE_MODE`, `developer_canary` or `same_publisher_batch`), `session_key` (`BROWSER_SESSION_REUSE_KEY`), `publisher_scope` (`BROWSER_SESSION_REUSE_PUBLISHER_SCOPE`), `ttl_seconds` (`BROWSER_SESSION_REUSE_TTL_SECONDS`), `base_dir` (`BROWSER_SESSION_REUSE_BASE_DIR`), `cleanup_expired` (`BROWSER_SESSION_REUSE_CLEANUP_EXPIRED`, default `true`), and `allow_cross_publisher` (`BROWSER_SESSION_REUSE_ALLOW_CROSS_PUBLISHER`, default `false`) control bounded reusable browser profiles for developer canaries and same-publisher batches. Same-publisher batch runs can also opt into `browser_download.warm_worker_pool.enabled` (`BROWSER_WARM_WORKER_POOL_ENABLED`, default `false`) with `max_workers`, `max_runs_per_worker`, `max_memory_mb`, `idle_ttl_seconds`, and `fallback_to_subprocess`; warm workers are accepted only for headless `same_publisher_batch` session reuse with cross-publisher reuse disabled, and the existing one-shot subprocess remains the fallback path.
- Browser downloads and normal report ingest also write into `paths.reports_db` table `report_sources`. Discovery inserts one Source row per new diff item with `source_status='discovered'`, publisher/source-page provenance, and `discovered_on_page_number`; a later successful local PDF download or downstream ingest upgrades the same normalized-URL row in place to `source_status='downloaded'`, filling `downloaded_at_utc` and the downloaded file `md5`. Successful downloads and ingested reports receive a typed deterministic report-value score over five dimensions (`market_insight_depth`, `evidence_specificity`, `decision_relevance`, `recency_timeliness`, `source_authority_originality`), with `report_value_score`, `report_value_band`, `report_value_score_json`, and `report_value_scored_at_utc` persisted back to the same Source row for later publisher-resource ranking. Drive-folder PDFs that do not expose an original publisher/source URL are still scored and persisted by using their canonical Drive file URL as the source identity. A Source remains an acquisition candidate until the downstream report pipeline produces a validated Report artifact.
...
- Cost and token tracking: `analysis.cost_ledger_path` (`COST_LEDGER_PATH`, default `./out/cost-ledger.jsonl`), `cost.daily_path` (`COST_DAILY_PATH`, default `./out/cost-daily.json`), `cost.usage_db_path` (`LLM_USAGE_DB_PATH`, default `./state/llm_usage.sqlite`), and `cost.pricing_path` (default `./src/config/llm-costs.yaml`) resolve once to normalized runtime paths. SQLite `llm_usage_events` is canonical; an omitted call ordinal is atomically allocated per logical provider-call stream, while a resolved ordinal or provider request ID preserves replay identity. Every row retains provider-call, parse, schema-validation, error-stage, and terminal-error-code outcomes; invalid terminal-taxonomy combinations are rejected. JSONL/daily files are deterministic compatibility projections with durable SQLite checkpoints; `llm_usage_ledger_service.reconcile_usage_export(..., repair=True)` rebuilds missing or altered exports and compares event count, input/output/cached-token totals, and estimated cost with an exact `1e-9` cost tolerance. Browser-use uses the same configured paths through a bounded nonblocking callback queue; shutdown closes intake first and returns written, pending, and dropped counts within `browser_download.accounting_flush_timeout_seconds` (default `5`).
...
- `OPENAI_API_KEY` for primary browser-download automation
- `OPENROUTER_API_KEY` for browser-download fallback automation
...
- Final `analysis_<mode>.json` snapshots preserve internal report payload metadata, including the active vector store ID, persisted analysis pack paths, and text extraction stats used by downstream diagnostics.
      - Durable checkpoints are written through `file_service` after `source_prepared`, `selection_complete`, `analysis_complete`, and `render_complete`. Each `PipelineStageCheckpoint` stores `schema_version`, pipeline/file/stage identifiers, artifact references, artifact MD5 integrity for existing file artifacts, and the semantic state needed for restart.
      - Semantic restart supports `resume_from_stage="source_prepared"`, `"selection_complete"`, `"analysis_complete"`, `"render_complete"`, and `"latest_safe"` through `run_report_generation(...)` and `run_report_pipeline(...)`. Checkpoint reads validate persisted artifact metadata before trusting the semantic state; direct corrupt, missing-artifact, hash-mismatch, and unsupported-stage requests fail with non-retryable `AppError`s, while `latest_safe` skips invalid newer checkpoints and resumes from the newest valid one. New checkpoints also persist a typed artifact registry with artifact IDs, kinds, paths, content hashes, producer steps, required flags, and timestamps; legacy checkpoint integrity maps remain readable.
    ...
    - **Categorization**: stored evidence packs are compacted into a `ReportCategoryContext`, then a single batched category-fit prompt evaluates all portal categories against that context using each category's `definition`, `include_when`, and `exclude_when` guidance from `src/config/category-mappings.yaml`. The selector returns at most two portal categories, which are exposed publicly as Topics. Taxonomy tags are kept as metadata and do not determine category assignment.
     ...
     - **Strict cropping**: final crops are routed by visual kind (`table_strict` for tables, `chart_strict` for charts) so each class uses tailored geometry tightening. Selected final table/chart crops render at `rank.final_crop_dpi` while discovery previews and crop-refine page context remain separately controlled by `ingest.contents_page.render_dpi` and `rank.crop_refine_page_dpi`. `publication_strict` is the public-facing mode: it maps tables/charts into those strict geometry paths, snaps high-confidence table outer rules, uses content-aware trim for shaded/dark/gradient panels, runs final rendered-PNG QA with table-boundary, chart-completeness, and visual-card-boundary detector diagnostics, attempts at most two score-improving content-aware repairs for clipped or contaminated edges, writes `.qa.json` diagnostics sidecars, and only returns accepted crop paths.
    ...
    - **HTML rendering**: `render_service` generates the final MarketBearing executive report page with semantic sections, SEO metadata (OG/Twitter/canonical/JSON-LD), responsive layout, compact report identity, report-value score, source-backed findings, optional visual-evidence carousel, expert commentary, collapsible LinkedIn copy, source/download links, and explicit image dimensions. The hero gives the report title full-width priority with the TLDR beneath it, Overview keeps the executive summary only, and section intros use value-led headings that span the available content width before their supporting cards or grids. The service reuses one module-scoped Jinja environment so repeated renders stay deterministic without rebuilding template state on every call.
...
7. **State record**
   ...
   - `run_report_pipeline(...)` passes a typed `ReportGenerationClientBundle` into report generation instead of inspecting the report function signature. The bundle carries source OCR, taxonomy, category-fit, evidence-pack, artifact, validation, regeneration, and figure-caption clients so model-client wiring is explicit at the pipeline boundary.
   - UI runs classify failed records into typed next actions such as retry, resume from checkpoint, credential request, transient cleanup, publish-only continuation, or permanent failure. `poll_ui_run(...)` returns the classification for UI display, and dead-letter auto-triage records the recommended action note.
   - Live verification on June 29, 2026 used existing PDFs under `out/9/` and real model/API calls. Two report-pipeline runs wrote typed registries for `source_prepared`, `selection_complete`, `analysis_complete`, and `render_complete`; `latest_safe` resume validated registry hashes and returned in 0.144s after a 523.947s fresh run and in 0.082s after a 501.629s fresh run. Both live runs surfaced an existing `card_publication_date_invalid` report-card validation block, so the registry/resume path avoided repeated model work while preserving the terminal validation error.
...
Retry behavior has one owner: orchestrators use `src/orchestrators/retry_orchestrator.py` for bounded workflow retries and backoff. `src/services/llm_service.py` performs exactly one primary provider attempt, disables OpenAI/OpenRouter SDK retries, retains circuit breaking and optional scope-level rate limiting, and immediately propagates typed failures. For `openai_chat_json`, a retryable OpenAI failure may make one bounded OpenRouter fallback attempt through `LLMProviderOperations.openrouter_chat_json`; both providers adapt into the same typed response contract, failover start/complete events are logged, and fallback exhaustion propagates the OpenRouter `AppError` back to the orchestrator. Nonzero `ingest.llm` retry/backoff configuration is rejected.
...
- Editorial artifact prompts now carry an executive-grade rubric: summaries must lead with the answer, quantify the most important movement, state the implication, and include a supported caveat only when present; expert comments and LinkedIn posts receive `metric_spine_json` plus claim-level evidence-bearing summary/insight JSON and must not expose evidence IDs, evidence spans, page references, JSON fields, or internal pack structure.
...
- `llm.py`: canonical provider-operation contract for OpenAI/OpenRouter chat JSON, image, OCR, and vector-store calls.
...
The repo-level report download automation uses that same local runtime through `src/services/browser_report_download_service.py`. It now plans each attempt from remembered route memory plus discovery/diff evidence, probes candidate PDFs before browser-use when discovery already exposed them, tailors browser-use prompts per route family, captures structured route steps plus blocker/terminal/on-site evidence, and stores both the best legacy projection and the richer per-attempt route history for later reuse. Verified or recovered successful browser-route results can also be promoted into reviewable YAML playbooks under `src/playbooks/browser_routes/` when `browser_download.route_playbook_promotion_mode` is `canary`, `dry_run`, or `write`; the orchestrator logs the selected mode, skip reason or playbook path, version, review-required status, and review-diff line count. Private-API playbook promotion is automatic when `browser_download.private_api_playbook_promotion_mode` is `canary`, `dry_run`, or `write`: the orchestrator inspects verified downloaded browser results, asks the browser-download service to replay safe endpoint candidates, records threshold progress in `publisher_private_api_candidates`, and writes YAML only after `private_api_playbook_min_success_count` and `private_api_playbook_min_distinct_source_urls` are both met in write mode. The manual `python -m src.cli promote-private-api-playbook --request-json <path>` command remains available for reviewed backfills and one-off operator promotions.
...
Browser-download automation builds `ChatOpenAI` from `browser_download.model` first and passes `ChatOpenRouter` from `browser_download.openrouter_model` as `fallback_llm` when `OPENROUTER_API_KEY` is configured. For direct OpenRouter examples, see `tools/browser-use/examples/models/openrouter.py`.
...
Run the production-like autonomous happy-path smoke suite locally:

```bash
python scripts/quality/autonomous_happy_path_smoke.py --output out/autonomous_happy_path_smoke.json
```

Refresh the capability-level observability and architecture map:

```bash
python scripts/quality/capability_maps.py
```
...
- `python scripts/ci/check_coverage.py --coverage-xml coverage.xml` (global + per-critical-package thresholds, plus stricter `COVERAGE_CONTROL_PLANE_MIN` coverage for new control-plane interconnection modules such as pipeline preflight and retry telemetry)
- `python scripts/ci/run_mutation_gate.py --json-out mutation_results.json` (mutation score gate for critical generators/services/orchestrators, including targeted planner/preflight/retry-telemetry control-plane logic)
...
- `python scripts/ci/check_pdf_candidate_benchmark.py --baseline docs/quality/pdf_candidate_extraction_benchmark_baseline.json --output-json out/pdf_candidate_benchmark.json --fail-on-runtime-regression` (strict local/release PDF candidate benchmark gate; uses the committed, SHA-256-verified PDFs under `tests/fixtures/pdf_benchmark/golden/` and fails on source-hash, candidate signature/count/degraded-page, or documented runtime-threshold drift)
- `python scripts/ci/check_pdf_crop_refine_benchmark.py --baseline docs/quality/pdf_crop_refine_benchmark_baseline.json --output-json out/pdf_crop_refine_benchmark.json --fail-on-runtime-regression` (strict local/release PDF crop-refine benchmark gate; fails on candidate-pack, crop-refine decision, crop artifact, crop count, or estimated model-work drift and on documented runtime failure thresholds when existing generated artifacts under `out/` are present)
- `python scripts/ci/check_pdf_benchmark_trends.py --candidate-json out/pdf_candidate_benchmark.json --crop-refine-json out/pdf_crop_refine_benchmark.json --history-json out/pdf_benchmark_trend_history.json --output-json out/pdf_benchmark_trends.json --update-history --fail-on-trend-regression` (strict local/release PDF benchmark trend gate; consumes existing benchmark JSON outputs without rerunning extraction and fails sustained runtime or crop-refine model-work regressions over retained history)
...
- `python scripts/quality/retry_decision_telemetry.py <log_path> --output-json out/retry_decision_telemetry.json --max-exhaustion-rate 0.20` (deterministic retry/defer/user-action telemetry report from structured logs; exits nonzero when retry-exhaustion rate breaches the supplied threshold)
- `python scripts/quality/run_health_scorecard.py <log_path> --run-id <run-id> --max-retry-exhaustion-rate 0.20` (local/live-run health summary for latency, retries, validation failures, cost warnings, and attached retry-decision telemetry)
- `python scripts/quality/run_health_scorecard.py <log_path> --run-id <run-id> --pdf-candidate-json out/pdf_candidate_benchmark.json --pdf-crop-refine-json out/pdf_crop_refine_benchmark.json --pdf-trend-json out/pdf_benchmark_trends.json --require-pdf-benchmark-evidence --output-json out/run_health_scorecard.json` (release health scorecard with retained PDF benchmark evidence; summarizes candidate counts, degraded-page counts, crop/refine artifact and decision counts, estimated model-call drift, runtime drift, and trend rows without rerunning PDF benchmarks)
- `python scripts/quality/release_evidence_manifest.py --release-id <release-id> --artifact coverage=coverage.xml --expected-schema coverage=<coverage-version> --artifact mutation=mutation_results.json --expected-schema mutation=1.0 --artifact health_scorecard=out/run_health_scorecard.json --expected-schema health_scorecard=1.0 --fresh-after <gate-start-iso> --require-head-commit --allow-issues --output-json out/release_evidence_manifest.json` (release evidence bundle manifest; validates retained artifact presence/schema versions/freshness, records commit SHA, command arguments, artifact hashes/paths, timestamps, and pass/fail status, and writes failed manifests for review when `--allow-issues` is supplied)
- `python scripts/quality/release_evidence_review.py --manifest-json out/release_evidence_manifest.json --waivers-yaml docs/quality/release_evidence_waivers.yaml --output-json out/release_evidence_review.json --output-md out/release_evidence_review.md` (release evidence review gate; writes deterministic JSON and Markdown grouped by artifact status, freshness timestamp, issue reason, and waiver state, and exits nonzero for unwaived manifest issues or invalid waivers)
...
- PDF candidate performance/equivalence baseline: `docs/quality/pdf_candidate_extraction_benchmark_baseline.json` references the committed, SHA-256-verified PDFs in `tests/fixtures/pdf_benchmark/golden/`. CI runs the gate without `--allow-missing-assets`; missing or hash-mismatched PDFs fail. Refresh the baseline only after an approved candidate-output change by running `python scripts/quality/pdf_candidate_benchmark.py --baseline docs/quality/pdf_candidate_extraction_benchmark_baseline.json --output-json out/pdf_candidate_benchmark_refresh.json --write-current-baseline docs/quality/pdf_candidate_extraction_benchmark_baseline.json --iterations 2`; review source hashes, candidate signatures, counts, degraded-page counts, and runtime deltas before committing.
- PDF crop-refine artifact/cost baseline: `docs/quality/pdf_crop_refine_benchmark_baseline.json` references existing generated candidate packs, crop-refine caches, crop PNGs, and crop-refine page renders under `out/`. CI runs this gate with `--allow-missing-assets` because those artifacts are local/generated outputs, but release or local verification should omit that flag so missing artifacts fail and add `--fail-on-runtime-regression` when runtime failures must block release. The cost proxy is the cached crop-refine decision count plus estimated two-phase page model-call count derived from existing `crop_refine.json` decisions. Refresh the baseline only after an approved crop/refine output or cost-policy change by running `python scripts/quality/pdf_crop_refine_benchmark.py --baseline docs/quality/pdf_crop_refine_benchmark_baseline.json --output-json out/pdf_crop_refine_benchmark_refresh.json --write-current-baseline docs/quality/pdf_crop_refine_benchmark_baseline.json --iterations 3`; review candidate-pack signatures, crop-refine signatures, crop artifact hashes/counts, estimated model-call counts, and runtime deltas before committing.
- PDF benchmark trend history: `scripts/ci/check_pdf_benchmark_trends.py` reads existing candidate and crop-refine benchmark JSON outputs, appends a bounded local history at `out/pdf_benchmark_trend_history.json`, and writes a release report at `out/pdf_benchmark_trends.json`. The retained history is intentionally local/generated output, not a committed fixture. Use `--retained-run-limit` to prune old runs, `--min-runs` to require enough history before sustained-regression checks, and `--fail-on-trend-regression` in release verification. Trend waivers should be limited to documented machine changes, approved benchmark baseline refreshes, or intentional cost/latency tradeoffs recorded with the release evidence.
- PDF benchmark scorecard evidence: `scripts/quality/run_health_scorecard.py` can attach existing PDF candidate, crop/refine, and trend JSON outputs to the run health scorecard with `--pdf-candidate-json`, `--pdf-crop-refine-json`, `--pdf-trend-json`, and `--require-pdf-benchmark-evidence`. The scorecard does not rerun extraction; it reads retained benchmark outputs, records their paths in `pdf_benchmark_scorecard.source_files`, and writes a retained review artifact when `--output-json` is supplied. Release review should link the four retained files together: candidate benchmark JSON, crop/refine benchmark JSON, trend JSON, and health scorecard JSON. If any benchmark input is missing or empty, the scorecard marks `pdf_benchmark_scorecard.evidence_complete=false`, emits a `pdf_benchmark_evidence incomplete` warning, and exits nonzero so operators fix the evidence gap before approving the release.
- Release evidence bundle manifest: after release gates have produced retained outputs, run `scripts/quality/release_evidence_manifest.py` with one `--artifact name=path` and matching `--expected-schema name=version` per retained artifact. Include at least the coverage XML, mutation JSON, PDF candidate benchmark JSON, PDF crop/refine benchmark JSON, PDF benchmark trend JSON, and run health scorecard JSON. The manifest writes deterministic JSON with the current commit SHA, manifest command arguments, producer commands when supplied through `--artifact-command name=...`, artifact paths, schema versions, timestamps, byte counts, SHA-256 hashes, file modification timestamps, and extracted pass/fail status. CI records `RELEASE_EVIDENCE_STARTED_AT` before coverage generation, builds `out/run_health_scorecard_ci.json`, runs the manifest with `--fresh-after "$RELEASE_EVIDENCE_STARTED_AT"`, `--require-head-commit`, and `--allow-issues`, then runs `scripts/quality/release_evidence_review.py` as the approval gate. Local operators should use a gate-start timestamp from the current verification run; stale artifacts older than that timestamp, missing files, invalid JSON/XML, schema-version drift, commit-SHA mismatch, or failed/warned gate payloads become manifest issues that must be regenerated or reviewed before approval.
- Release evidence review and waivers: `scripts/quality/release_evidence_review.py` reads one or more `out/release_evidence_manifest*.json` files and writes both `out/release_evidence_review*.json` and `out/release_evidence_review*.md`. The Markdown is the operator review surface; it groups artifacts by status, required flag, issue counts, unwaived issue counts, and freshness timestamp, then lists every issue with waiver owner, expiry date, and justification when present. Waivers live in `docs/quality/release_evidence_waivers.yaml` with `artifact_name`, `reason`, `owner`, `expires_on`, and `justification`. CI fails on expired waivers, ownerless waivers, empty justifications, invalid expiry dates, unmatched waivers, and any unwaived manifest issue. Retire a waiver by regenerating the underlying evidence or fixing the failing gate, rerunning the review until the waiver becomes unmatched, and removing the waiver in the same change. CI uploads the manifest, review JSON, review Markdown, waiver file, and every retained gate artifact in the `release-evidence-bundle` artifact so waived issues remain visible.
...
- `x100tasks.md` (Notion-derived x100 intake list; items become active only after promotion into `CONSOLIDATED_TODO.md` with owner, baseline, target, and review/expiry date)
...
- Meaning: Drive returned a file with a PDF header but no `%%EOF` marker after bounded redownload attempts, so ingest treats the source artifact as structurally corrupt before report generation.
- Typical fix: replace or reacquire the source PDF in Drive. The failed `(file_id, md5)` is recorded in `state_db` and skipped on later runs; uploading a corrected file revision with a new MD5 makes it eligible again.
- Related code path: `src/orchestrators/ingest_file_orchestrator.py` records the permanent source-validation failure before returning the file-level error outcome.
...
The browser-download service now rejects weak remembered route summaries before they are stored for reuse, requires real delivery confirmation evidence before classifying an email-gated run as `email_requested`, filters obvious non-field controls out of browser identity auto-upserts, records blocker and terminal evidence in route history, and cross-checks PDF signature plus MIME/extension metadata before reporting `downloaded`. Confirmation scoring now accepts generic success states such as visible thank-you text only when combined with another independent browser signal, and routed form-heavy report pages are no longer forced into `browser_onsite_report` when the browser evidence clearly shows an email gate. Pre-browser PDF shortcut candidates are title-aware when the caller supplies a report title, so unrelated same-publisher PDFs are rejected before they can be archived as the requested report. Browser identity learning also filters runtime-only submit controls, CRM/internal field IDs, dropdown option dumps, login/password controls, and status text instead of promoting them into `src/config/browser_download_identity.yaml`. When a site opens a PDF viewer wrapper instead of writing the real PDF bytes directly, the service fetches the embedded real PDF and replaces the wrapper file before reporting success; when the artifact is actually a blocked form or an on-site longread, the service can now recover that terminal state instead of forcing a fake PDF success. Browser-download HTTP probes and direct artifact fetches now run through `src/services/_http_acquisition.py`, which reuses pooled per-host sessions, applies bounded response policies, and preserves typed route-specific failures in the acquisition logs. Failed report-download attempts now also emit a typed forensic pack JSON under the per-URL browser download root, carrying the failing route-plan step, classified error metadata, bounded terminal evidence, and either copied or metadata-only artifact references according to `browser_download.failure_forensics.policy`; expected blockers such as CAPTCHA, 403/access challenges, static email gates, and business-email rejection default to metadata-only forensics. Browser-agent execution-budget timeouts are retryable under the orchestrator's bounded retry policy, and Drive archival uploads only verified report artifacts rather than forensic screenshots or HTML snapshots.

When an explicit `--delivery-email` is supplied, static email-gate detection no longer stops at `email_required`; the planned browser email-form route is allowed to submit the form and verify `email_requested`. Without an explicit delivery email, `download-report` now uses the configured mailbox account address when one is available before falling back to identity-profile delivery emails; if no delivery address can be resolved, static email-gate detection still fails closed as `email_required` before browser spend. Deferred mailbox delivery requests are recorded only after confirmed `email_requested` outcomes, so unconfirmed `email_required` blockers do not pollute the mailbox queue.
...
Poll the configured mailbox for a delayed email-delivered report and continue acquisition through the existing download workflow:

```bash
python -m src.cli poll-mail-report https://example.com/report --report-title "Market outlook" --publisher-name "Example Publisher" --delivery-email analyst@example.com
python -m src.cli poll-mail-report https://example.com/report --report-title "Market outlook" --publisher-name "Example Publisher" --poll-timeout-seconds 120 --poll-interval-seconds 15
```

`poll-mail-report` is routed through `src/orchestrators/mail_report_acquisition_orchestrator.py`. It loads mailbox settings from `mailbox_acquisition` in `src/config/app.yaml` and mailbox credentials from `.env`, searches the mailbox with source URL, report title, publisher, and delivery-email context, selects only links with report/download/document evidence tied to the requested publisher or source host, materializes matching PDF attachments before link follow-up, and then calls `run_report_download` for the selected URL. Pass `--requested-after-utc` with the UTC time of the form submission when re-testing the same publisher/report so older matching delivery emails are ignored. Use `--poll-timeout-seconds` and `--poll-interval-seconds` to bound a one-off live poll without changing YAML defaults. Retryable candidate-download failures, such as a browser-agent timeout after a mail link is selected, surface as retryable workflow failures instead of spending the rest of the poll window on sibling links. Permanent candidate failures persist request-scoped rejection evidence in `mailbox_candidate_rejections`; later autonomous runs suppress the same rejected message/link host until the TTL expires. If the report email has not arrived within the configured polling window, the command raises retryable `mail_report_not_arrived_yet`; this is expected for publishers whose delivery queue takes several minutes. Deferred requests are processed without a CLI operator by `workflow_control_orchestrator.run_due_mail_delivery_requests(...)`, which lists due `mail_delivery_requests`, invokes the same mailbox acquisition orchestrator, carries seen provider IDs forward, and promotes successful mailbox delivery into publisher route memory so future `download-report` planning can choose that route before browser launch.
...
Any omitted browser settings fall back to the existing `browser_download` section so the discovery flow can share the same browser runtime defaults without duplicating mandatory configuration.
...
### Mailbox Acquisition Config

Email-delivered report acquisition is configured under `mailbox_acquisition` in `src/config/app.yaml`. Secrets stay in `.env`; YAML stores only non-secret behavior and local file paths.

Supported keys:

- `provider`: `imap`, `gmail`, or `auto`
- `output_dir`
...
For IMAP-backed mailboxes, set these `.env` values:
...
For Gmail OAuth, set `MAILBOX_PROVIDER=gmail` and configure `gmail_oauth_client_path` / `gmail_oauth_token_path`. When `provider` is `auto`, the service prefers IMAP when IMAP credentials are complete and otherwise tries Gmail when OAuth files are configured.
...
- LLM boundary: only canonical `src/services/llm_service.py` exposes provider operations; generic policy/client composition, OpenAI client/cache/metadata, chat/analyze, response/OCR, OpenRouter client construction/fallback, and vector-store provider operations live under `src/services/_llm_service/*`. Usage accounting is emitted as a typed `OpenAIUsageAccountingRequest` and persisted through `src/services/openai_accounting_service.py`, keeping JSONL and SQLite usage-ledger side effects outside the provider client boundary.
...
- Artifacts: `src/generators/artifact_generator.py` writes `artifacts.json` under the same analysis path, parallelizing independent steps with dependency ordering and process-wide rate limiting via `ingest.artifacts.*`. The user-facing artifact contract includes optional `metric_spine` records derived from supported key metrics, optional `executive_advisory` decision-brief/recommendation/risk sections, `topics_covered` topic modules, `key_figures` quantified proof points, `chart_insight_cards` with confidence/avoidance metadata, and `claim_ledgers` with canonical claim IDs, artifact sections, evidence IDs, support type, confidence, and risk. Artifact prompts include the shared Market Lense editorial constitution through the prompt service, and final editorial prompts receive the same metric spine plus evidence-span-bearing summary and insight JSON so public copy can use quantified proof without exposing internal evidence IDs.
- Cost ledger: `src/services/llm_usage_ledger_service.py` rebuilds JSONL entries and daily rollups (`./out/cost-ledger.jsonl`, `./out/cost-daily.json`) from canonical SQLite after each newly recorded LLM event, using per-model pricing from `src/config/llm-costs.yaml` or the configured `cost.pricing_path`. Provider-reported prompt-cache tokens are retained with a `provider_hit` cache outcome, so canonical/export reconciliation includes cached-input totals. `src/services/cost_ledger_service.py` remains the compatibility read/report service for those projections.

For repeatable read-only CTO evidence extraction from retained local state, run `python scripts/quality/collect_cto_review_evidence.py --output-dir out/cto-review-evidence`. It neither contacts external systems nor executes acquisition or publishing workflows.
