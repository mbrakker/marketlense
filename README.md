...
- Browser acquisition is governed by `browser_download.run_budget` in `src/config/app.yaml`. Before each governed browser attempt, the download orchestrator reads the canonical `llm_usage.sqlite` ledger across the exact run, UTC day, and publisher scopes and applies the most conservative metric value. The default profile caps browser launches and PDFs at 30 and uses the existing daily spend hard stop; it is recorded in the shared ledger rather than a second accounting database.
