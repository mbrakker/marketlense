...
- OpenAI Responses compatibility: known unsupported params (for example `temperature`/`seed` on `gpt-5*`) are omitted before image and vector-backed Responses API calls. Unknown model/parameter mismatches fail once as typed non-retryable bad requests instead of issuing an unowned second provider call.
