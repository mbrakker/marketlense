...
- Browser-report HTTP isolation: `src/services/_browser_report_download/http.py` remains the private compatibility surface, while PDF transfer, page-PDF probing, gate probing, onsite capture, and shared HTTP/HTML evidence live in `_http/` capability modules behind the canonical browser-report service boundary.
