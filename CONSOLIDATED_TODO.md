...
- **What to implement, in order:**
  ...
  - **Items 7–9 progress (2026-09-26):** Model-assisted repairs now return a validated private decision and minimal patch in the same provider call. The retry loop records typed monotonic deltas and the actual strategy/evidence fingerprint, retries only from promoted artifacts, rejects repeated candidate hashes for unchanged input/validator identity, and ends on strategy exhaustion rather than attempt-number rules. Candidate audits retain private decisions and retry deltas; public artifacts and the read-only scorecard do not expose repair prose. Item 10's hash-pinned historical real-failure benchmark remains outstanding.
  - **Workflow verification update (2026-09-26):** One isolated IAS run used the canonical production queue from the retained frozen PDF through `awaiting_review`; discovery, candidate qualification, source preparation/validation, analysis, rendering, final HTML validation, and publication preflight succeeded. Validation and readiness passed in one attempt (40 provider calls, $0.049622); publication stayed disabled. This did not exercise live publisher-inventory crawling or a repair attempt, so item 10 remains necessary for real-failure effectiveness evidence.
