...
- **Title:** Automatically enqueue delayed mailbox polling after verified email-requested report submissions [Impact: 5/5, Effort: 3/5]
  - Problem fixed: Mailbox polling can now materialize delayed email-delivered reports, but report-download and acquisition-audit flows still require an explicit follow-up command after an `email_requested` outcome.
  - Why implement: Removes operator coordination from the highest-friction gated-report path and lets delayed publisher emails become acquired artifacts as soon as they arrive.
  - Tradeoffs / risks: Needs idempotent submission/poll correlation so reruns do not submit duplicate forms or download unrelated newsletter links.
  - Acceptance Criteria:
    - Verified `email_requested` outcomes persist a deferred mailbox-acquisition job with source URL, publisher, report title, submitted delivery email, route history ID, and idempotency key.
    - A workflow-control worker polls due mailbox jobs with bounded backoff, reuses `run_mail_report_acquisition`, and transitions not-yet-arrived mail to deferred retry instead of failure.
    - Successful delayed mail acquisition updates the existing `report_sources` row and route history without creating duplicate report-source records.
    - Live verification covers one real delayed email request through automatic polling, no operator command, and no duplicate submission on rerun.
    - Tests cover enqueue idempotency, retryable not-arrived state, stale job expiry, unrelated-mail rejection, successful materialization, and required structured logs.

- **Title:** Auto-skip or route business-domain-only gated forms before mailbox polling [Impact: 4/5, Effort: 2/5]
  - Problem fixed: Several live gated publishers reject the configured mailbox as non-business before a report email can be requested, but the acquisition flow still spends browser time discovering that blocker per URL.
  - Why implement: Converts a repeated external constraint into fast, unattended route policy so the system avoids mailbox polling that cannot succeed and spends browser budget on publishers where delivery is possible.
...
- **Title:** Learn and apply safe required-select identity values from live gated-form evidence [Impact: 5/5, Effort: 3/5]
  - Problem fixed: The 2026-07-04 database-derived email-route matrix blocked 4 of 10 tested publishers on required enum/select fields whose option sets were visible but not represented in the configured identity profile.
