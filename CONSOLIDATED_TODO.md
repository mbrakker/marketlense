...
- **D10 — Browser executor, static DOM scan, and prompt-payload reduction (2026-08-20):** The reused async Browser Use session now runs the full deterministic playbook action set—navigation, click/submit, fill/type, select, and verify—using the same safe locator and `${identity.*}`-only value rules as the synchronous executor. Each action verifies its URL/text postcondition; locator/control/postcondition drift returns to the existing Agent on the same browser/page/cookie/storage state. The focused affected suite passed 191 tests in 162.398 seconds (two unrelated pre-existing async-autofill expectation conflicts excluded); its success case proves 0 Agent calls and one browser start. The guarded real Browser Use integration passed 1/1 in 74.402 seconds. The isolated discovery-to-publish profile correctly prevented further workflow progress because it lacks a configured Drive parent; no publish write was possible (`max_wordpress_writes: 0`).
