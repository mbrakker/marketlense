...
- **What to implement, in order:**
  ...
  - **Foundation progress (2026-09-26):** This change implements the deterministic/atomic foundation in items 1–6: retained-claim diagnostics run before repair and on candidates, repairs resolve and merge at exact item/field scope, verified derived changes are audited, deterministic correction paths precede model repair, and alternative evidence uses the existing typed compatibility scorer. The expanded affected suite passes. The isolated discovery-to-publish workflow remains unverified because discovery stopped with `publisher_inventory_coverage_invalid` (the current inventory was materially smaller without a newly qualified report asset); E13 remains active for its retained real-failure benchmark and outstanding workflow evidence.
