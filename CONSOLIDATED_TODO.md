...
- **Task 7 — Category-fit consistency closure (2026-07-26):** Category fit now uses the mapping-owned strict high-confidence threshold, stable rule IDs, explicit semantic concepts, retained evidence references, and title/overview/key-finding centrality. A high-fit rejected category with supported inclusion, centrality, and no evidenced exclusion is deterministically selected; explicit central exclusions reject; unresolved cases receive exactly one candidate-only repair and then fail closed. Selected IDs are derived from normalized decisions. The retained regression corpus covers high fit, the historical Activate Social Video contradiction, creator/influencer, social commerce, secondary creator content, explicit exclusion, and low fit. A live GPT-5-mini replay of the retained Social Video context returned `social_video_creator` at 0.95 as primary with one supported rule, no exclusion, centrality evidence, and agreeing selected IDs. A fresh isolated ingest made real provider calls, deterministically selected `social_video_creator`, routed only an unresolved `consumer_behavior` candidate to its one targeted repair, then correctly failed closed; safe publish preflight made zero WordPress writes.
