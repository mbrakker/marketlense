...
| Active | A18 | Harden discovery recall and authoritative acquisition handoff | Establish ground-truth recall, make candidate decisions reversible, and remove conflicting report reclassification before acquisition. |
...
5. **Make discovery qualification authoritative at the acquisition handoff.** A qualified report source must not be rejected again by an independent report-likelihood heuristic. Keep `candidate_download_readiness` only for ad-hoc/direct URLs that bypass discovery, and reduce duplicated report-marker policy across discovery, qualification, readiness, and route planning. Normalize terminal acquisition semantics so confirmed 403/404/static-archive outcomes are access/unavailable states rather than `email_delivery` classifications.
6. **Close secondary acquisition correctness gaps without broad redesign.** Revalidate mutable artifact-cache entries with cheap remote freshness evidence when available while preserving long reuse for immutable/versioned sources; reserve budget by actual route operation rather than treating every attempt as one PDF process; and distinguish local acquisition from required durable archival so final acquisition completion cannot be recorded before mandatory Drive persistence succeeds.
...
- First-run publisher discovery requires a bounded completeness proof (for example explicit terminal pagination/declared total/structured source agreement or one verification browser pass) before its snapshot becomes the long-lived baseline.
...
- A mutable-URL cache fixture cannot reuse an outdated artifact after remote freshness evidence changes; immutable cache reuse remains zero-provider where safe. Route-specific budget tests prove an email-form/onsite route is not blocked solely by an exhausted PDF-processing allowance when no PDF work is attempted.
- Required Drive archival failure leaves the source in a recoverable locally-acquired/not-durably-archived state rather than a completed acquisition state; replay converges idempotently after storage recovers.
...
- The active backlog contains 29 outcome-owned items. Deferred, closed, and excluded rows remain visible above; any newly discovered work must be merged into an existing outcome or justified as a new one.
