...
- **What to implement, in order:**
  ...
  - **Atomic writable-path correction (2026-09-26):** The planner now resolves exact retained scalar leaves before a model call, and that same set governs the model contract, patch validation, scope validation, and protected-field complement. Multi-field repairs declare multiple leaves; if any issue in an atomic target cannot resolve, the whole target abstains instead of falling back to family scope. Ambiguous, non-scalar, or empty writable/protected partitions stop before provider use. Verified deterministic dependent-artifact changes remain explicitly audited and are not model-writable. The frozen seven manifests are unchanged; exact-SHA replay and the isolated discovery-to-publish canary are pending for this implementation.
  ...
  - **Current measurement and closure update (2026-09-26; exact implementation SHA `baad5a69d9dc63c6a0b9e807c97fa1aea0f8bff3`):** All seven schema 2.0 cases now reconstruct complete production payloads and pass the canonical preflight; 173 historical case fields and original input references match the source manifests with zero mismatches. All seven production repair runs ended in failure (0/7 case outcomes successful). A21 produced five candidate audits across three of five reports; its canonical comparison denominator is incomplete and identity comparison is incompatible. The five audited attempts report five hard-failure introductions, five out-of-scope mutations, and five unsupported-evidence introductions. Mobile and DoubleVerify stopped on `regeneration_repair_decision_invalid` / `protected_field_changed` before any candidate audit, leaving both scorecards unavailable. The exact-SHA discovery-to-publish canary failed twice: structured-output recovery failed for `insights_candidates`, then the report-pipeline mutation-scope guard rejected `changed_path_outside_allowed_paths`. E13 remains Active because success, complete comparable scorecards, safety thresholds, residual-odds improvement, and the required canary pass are not demonstrated. Full per-case results, scorecards, manifest hashes, commands, usage, and check outcomes are retained in [`the current measurement evidence`](docs/quality/e13-repair-benchmark/results/2026-09-26-baad5a69.json) (SHA-256 `ef963b0cdc12341dc22f73aa4a4600b9b571aad6d8c4243abc3776a52564b88d`).
