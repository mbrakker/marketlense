...
- **What to implement, in order:**
  ...
  - **Atomic writable-path correction (2026-09-26):** The planner now resolves exact retained scalar leaves before a model call, and that same set governs the model contract, patch validation, scope validation, and protected-field complement. Multi-field repairs declare multiple leaves; unresolved, ambiguous, non-scalar, or empty partitions stop before provider use. Verified deterministic dependent-artifact changes remain explicitly audited and are not model-writable. The frozen seven manifests are unchanged; exact-SHA replay and the isolated discovery-to-publish canary are pending for this implementation.
