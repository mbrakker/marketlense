...
- **Baseline:** Durable queues, checkpoints, lineage, retry ownership, reliability-transition telemetry, publish readiness, approval, idempotent WordPress writes, and exact-HEAD three-report canaries are implemented. The latest valid frozen 20-report baseline is retained at measured SHA `bfab37bbd1e4c194c027f14dd54817fb61f940a6`: 20/20 were admitted, 2/20 (10%) reached `awaiting_review`, 18/20 (90%) failed, 20/20 reached typed terminal outcomes, and there were 0 operator interventions. Its current failure Pareto is `schema_reference_missing` (7), `workflow_queue_report_stage_failed` (5), `artifact_structured_output_invalid` (2), `soft_copy_claim_provenance_binding_invalid` (2), `soft_copy_claim_provenance_bindings_missing` (1), and `evidence_pack_invalid_json` (1). This valid evidence establishes the deficient first-attempt baseline, not SLO attainment. A21 owns shared-cause remediation; use E13 only for item-scoped candidate-regeneration repair where applicable. E11 remains closed and is reassessed only if later root-cause evidence contradicts its retained closure evidence.
...
- **What to implement, in order:**
  ...
  - **Foundation progress (2026-09-26):** This change implements the deterministic/atomic foundation in items 1–6: retained-claim diagnostics run before repair and on candidates, repairs resolve and merge at exact item/field scope, verified derived changes are audited, deterministic correction paths precede model repair, and alternative evidence uses the existing typed compatibility scorer. The expanded affected suite passes. The first direct publisher-inventory probe stopped with `publisher_inventory_coverage_invalid` because the current inventory was materially smaller without a newly qualified report asset.
  - **Items 7–9 progress (2026-09-26):** Model-assisted repairs now return a validated private decision and minimal patch in the same provider call. The retry loop records typed monotonic deltas and the actual strategy/evidence fingerprint, retries only from promoted artifacts, rejects repeated candidate hashes for unchanged input/validator identity, and ends on strategy exhaustion rather than attempt-number rules. Candidate audits retain private decisions and retry deltas; public artifacts and the read-only scorecard do not expose repair prose. Item 10's frozen benchmark and scorecard extension are implemented; complete historical measurement and closure evidence remain outstanding.
  - **Atomic writable-path correction (2026-09-26; exact implementation SHA `9120a8fee9289c3850e9f91048a22c9da5457038`):** The planner now resolves exact retained scalar leaves before a model call, and that same set governs the model contract, patch validation, scope validation, and protected-field complement. Multi-field repairs declare multiple leaves; if any issue in an atomic target cannot resolve, the whole target abstains instead of falling back to family scope. Ambiguous, non-scalar, or empty writable/protected partitions stop before provider use. Verified deterministic dependent-artifact changes remain explicitly audited and are not model-writable. The unchanged seven manifests were replayed on this SHA; the three original terminal repair-decision guard classes fell to zero, but candidate-level mutation-scope failures remain and no benchmark repair succeeded. The isolated discovery-to-publish canary passed with publication disabled. See the exact outcomes below and the [retained benchmark record](docs/quality/e13-repair-benchmark/results/2026-09-26-9120a8fe.json).
  - **Prior workflow verification (2026-09-26):** An isolated IAS run used the canonical production queue from the retained frozen PDF through `awaiting_review`; discovery, candidate qualification, source preparation/validation, analysis, rendering, final HTML validation, and publication preflight succeeded. Validation and readiness passed in one attempt (40 provider calls, $0.049622); publication stayed disabled. The subsequent exact-SHA canary outcomes and their measured usage are recorded below. This workflow did not exercise a repair attempt.
  - **Measurement and closure update (2026-09-26; exact implementation SHA `09df2bac76111e44b8e811337c83a1995c791954`):** The three hash-pinned manifests retain 7 cases across all required classes; the historical baseline has 0/7 valid success@3 and 19 rolled-back attempts, with incompatible cohort identities and unbounded residual baseline odds. A same-SHA replay stopped before candidate validation because all 7 retained inputs lack the pre-repair figure title/evidence fields required by the canonical completeness gate. The attempted first case used 3 calls, 9,238 input tokens, 2,079 output tokens, and USD 0.001963, but produced no scorecard. Current success, hard-failure rate, mutation/evidence safety, repetition, mode share, per-class results, and comparable usage were unavailable at that revision. That replay kept E13 Active pending complete pre-repair payloads and an unchanged-corpus measurement; see [`E13 measurement evidence`](docs/quality/e13-repair-benchmark/README.md).
  - **Prior measurement and closure update (2026-09-26; exact implementation SHA `baad5a69d9dc63c6a0b9e807c97fa1aea0f8bff3`):** All seven schema 2.0 cases reconstructed complete production payloads and passed the canonical preflight; 173 historical case fields and original input references matched the source manifests with zero mismatches. All seven production repair runs ended in failure (0/7 case outcomes successful). A21 produced five candidate audits across three of five reports; its canonical comparison denominator was incomplete and identity comparison incompatible. The five audited attempts reported five hard-failure introductions, five out-of-scope mutations, and five unsupported-evidence introductions. Mobile and DoubleVerify stopped on `regeneration_repair_decision_invalid` / `protected_field_changed` before any candidate audit, leaving both scorecards unavailable. The exact-SHA discovery-to-publish canary failed twice: structured-output recovery failed for `insights_candidates`, then the report-pipeline mutation-scope guard rejected `changed_path_outside_allowed_paths`. Its result remains retained in [`the prior measurement evidence`](docs/quality/e13-repair-benchmark/results/2026-09-26-baad5a69.json) (SHA-256 `ef963b0cdc12341dc22f73aa4a4600b9b571aad6d8c4243abc3776a52564b88d`).
  - **Current measurement and closure update (2026-09-26; exact implementation SHA `9120a8fee9289c3850e9f91048a22c9da5457038`):** The unchanged seven frozen cases passed production payload reconstruction and preflight; 6/7 reached candidate validation and produced 12 candidate audits. Strict valid success@1/@3 is 0/6 in the incomplete scorecard denominator and full case outcome is 0/7. The terminal `changed_path_outside_allowed_paths`, `patch_target_not_atomic`, and `protected_field_changed` repair-decision guard classes are each 0, with no gate weakened. However, the unchanged runtime scope validator recorded 56 out-of-scope path occurrences and the scorecard classified 8/12 candidate attempts out of scope; one repair decision failed closed with `protected_fields_incomplete`. The candidate audits introduced 78 hard failures over 8/12 attempts and unsupported evidence over 7/12. Mobile stopped before candidate validation at `regeneration_target_item_unchanged`; ebook ended at `schema_type_mismatch`; final-web and DoubleVerify exhausted their bounded attempts; the g0 case retained `protected_fields_incomplete`. Two cases ended with full validation status `pass` but did not meet strict scorecard success. The exact-SHA IAS canary passed validation and readiness in one isolated run to `awaiting_review`, with publication disabled; it does not exercise repair. Cohort comparison/scorecard denominator and repair-attributed usage remain incomplete or unavailable, safety thresholds are unmet, and residual odds remain unbounded, so E13 stays Active. Per-case outcomes, audit hashes/summaries, scorecards, exact commands, usage, canary, and check results are in [`the current measurement evidence`](docs/quality/e13-repair-benchmark/results/2026-09-26-9120a8fe.json) (SHA-256 `56bdc065fcfade8e0e1c87cca6595bc4bd455b261de986233cddeb8c8e41f797`).
