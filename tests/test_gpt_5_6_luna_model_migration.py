...
def test_canonical_configuration_routes_every_generative_call_to_gpt_5_6_luna() -> (
    None
):
    ...
    assert (
        configured_models["browser_download.openrouter_model"]
    ...
    assert all(
        policy["model"] == LUNA_MODEL
        and policy["pricing_key"] == LUNA_MODEL
...
def test_example_configuration_does_not_reintroduce_a_non_luna_llm_route() -> None:
    config = yaml.safe_load(
        (REPO_ROOT / "src" / "config" / "app.example.yaml").read_text(
            encoding="utf-8"
        )
    )
