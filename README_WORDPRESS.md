...
- The Reports archive pairs the shared hero with the reusable `[ml_report_browser]` shell. The sticky search toolbar spans the full report frame with selected-filter chips directly below the search row, while compact pictogram sorting and the sticky filter rail stay plugin-owned so archive, search, category, and publisher views share the same live dependent-facet behavior.
