...
- `[ml_archive_hero context="..."]`: canonical premium hero for the Publishers, Reports, Topics, Signals, and Briefings directory/archive routes. The shortcode owns the shared structure and closed context configuration; `.ml-archive-hero` in `assets/css/theme.css` owns the shared presentation.
...
- Publishers, Reports, Topics, Signals, and Briefings use the shared premium `[ml_archive_hero]` component. Each context renders four live counters in this order: Publishers (`publishers`, `reports`, `regions`, `topics`), Reports (`reports`, `publishers`, `topics`, `regions`), Topics (`topics`, `reports`, `publishers`, `regions`), Signals (`signals`, `reports`, `topics`, `publishers`), and Briefings (`briefings`, `reports`, `publishers`, `topics`). Change common markup or context copy in `render_archive_hero()` and change the platform-wide visual treatment once under `.ml-archive-hero` in the theme stylesheet.
