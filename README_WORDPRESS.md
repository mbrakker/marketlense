...
- Topic and publisher directories aggregate only entities represented by published reports, briefings, or signals. Legacy sentinel metadata such as `...` and `Not extracted` is omitted from public presentation. The publisher directory uses the same archive metric shortcode and canonical report browser/card renderer for its hero metrics and latest-report strip; publisher cards remain sourced from taxonomy records and published-content counts.
