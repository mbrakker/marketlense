...
- `[ml_featured_digest]`: compatibility shortcode name for the homepage Featured Report module backed by the latest published report.
...
- `[ml_briefings_index]`: canonical Briefings landing surface for `/briefings/`. Renders published Briefings in the shared filtered archive browser or an explicit institutional empty state.
- `[ml_signals_index]`: canonical Signals landing surface for `/signals/`. Renders published Signals in the shared filtered archive browser and otherwise reuses source-backed metrics from published report artifacts.
...
- `[ml_archive_hero context="..."]`: canonical premium hero for the Publishers, Reports, Topics, Signals, Briefings, and Methodology routes. The shortcode owns the shared structure and closed context configuration; `.ml-archive-hero` in `assets/css/theme.css` owns the shared presentation.
...
- Publishers, Reports, Topics, Signals, Briefings, and Methodology use the shared premium `[ml_archive_hero]` component. Each context renders four live counters in this order: Publishers (`publishers`, `reports`, `regions`, `topics`), Reports (`reports`, `publishers`, `topics`, `regions`), Topics (`topics`, `reports`, `publishers`, `regions`), Signals (`signals`, `reports`, `topics`, `publishers`), Briefings (`briefings`, `reports`, `publishers`, `topics`), and Methodology (`reports`, `publishers`, `topics`, `regions`). Change common markup or context copy in `render_archive_hero()` and change the platform-wide visual treatment once under `.ml-archive-hero` in the theme stylesheet.
- `[ml_report_browser]`, `[ml_briefings_index]`, and `[ml_signals_index]` render the common filtered archive browser: sticky live search, selected-filter chips, compact sort controls, dependent facets, current-view count, pagination, and the canonical compact card variant for the selected content type. The sticky search toolbar spans the full archive frame while compact pictogram sorting and the sticky filter rail stay plugin-owned.
- Topic and publisher directories aggregate only entities represented by published reports, briefings, or signals. Legacy sentinel metadata such as `...` and `Not extracted` is omitted from public presentation. The publisher directory retains the canonical report browser/card renderer for its latest-report strip, using the same small report-card size as the Reports archive; publisher cards remain sourced from taxonomy records and published-content counts.
- Native WordPress categories are the canonical public Topic surface. Category descriptions and `ml_topic_*` term meta are written from `src/config/category-mappings.yaml`, then rendered by the topic directory and category archives as approved definitions plus inclusion/exclusion rules.
- Topic archives fall back to published report cards when no reports exist for the selected topic, without merging or rewriting taxonomy identities.
