...
would otherwise exceed the Windows-safe path budget, their report directory
are deterministically compacted with a content-derived suffix. Concurrent
workers may produce the same eligible artifact, so each sidecar write uses a
unique temporary file followed by an atomic replace. A bounded retry absorbs
