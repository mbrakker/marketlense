...
its separately retained provider rate card. `text-embedding-3-large` at 1,024 dimensions is the dedicated claim
...
`ingest.validation.public_editorial_quality.disabled_rule_waivers` is the temporary staged-rollout escape hatch for the deterministic public-editorial release gate. Each mapping key is a stable rule ID and each value must be a concrete non-empty release-waiver reason. An empty or malformed entry has no effect; do not use this setting to suppress an unresolved reader-facing defect. It cannot suppress hard source-fidelity failures (for example, unsupported numeric claims or source identity mismatches); a repairable prose-only rule such as a sentence fragment remains eligible for a bounded regeneration attempt or an explicitly recorded temporary waiver.
...
`ingest.run_budget` sets retry and elapsed-runtime safeguards; it does not cap the number of PDFs. `browser_download.run_budget` and `publish.run_budget` remain the scoped acquisition and publication controls; all use the same `cost.usage_db_path` authority. The isolated browser-agent subprocess rehydrates the complete serialized `browser_download` settings contract, including Drive archive settings, isolated cost and usage paths, and scoped run-budget limits; it must never fall back to process-default accounting paths. Vector-store requests, including readiness-status polling and metadata updates, inherit the active report runtime's `RunBudget`, so their forecasts and actual usage are written to that same isolated ledger rather than a process-default ledger. `browser_download.route_suppression` is independently reversible: it requires a minimum of three compatible typed terminal failures, records a policy-compatibility hash and TTL, and can always be bypassed with the explicit acquisition revalidation option. It never permanently blacklists a publisher.

Optional numeric values in the serialized browser-worker configuration accept JSON numeric or string scalars only. Missing values remain unset; arrays, mappings, and other non-scalar values fail locally with a typed configuration error before browser work begins.

Publisher-inventory browser discovery applies its configured traversal timeout and also bounds final browser cleanup. A stalled browser shutdown is recorded as `publisher_inventory_browser_kill_failed` and cannot indefinitely hold the discovery workflow.
