...
For a paced live run, set `publish.publish_interval_seconds` in the isolated run profile. The publisher measures the interval from the start of one WordPress write to the start of the next, including after a failed write attempt; idempotent existing-post readbacks do not consume a write interval. The base profile defaults to `0` so ordinary sandbox tests remain fast.

The REST transport disables redirect following. Any runtime redirect to `wp-admin/install.php` or `wp-admin/setup-config.php` is recorded as `wordpress_target_installation_redirect`, is non-retryable, and immediately terminates the publish run. Repair the sandbox target and repeat preflight before resuming; do not continue publishing the cohort against a setup redirect.
