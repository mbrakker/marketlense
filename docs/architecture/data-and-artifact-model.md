...
Bounded validation and release canaries retain one canonical manifest in the reports database. A manifest is created with immutable configuration, policy, workflow-run, and producer-build identities. Each entity attempt retains its stage, artifact IDs, timing, typed result, retryability, repair and idempotency dispositions. Superseded attempts remain auditable while exactly one current attempt carries the entity terminal outcome. The manifest audit derives stage totals and the final validation cohort from durable rows; release evidence must use that audit rather than reconstructing a funnel from logs.
