...
- [ ] Add a structural test requiring shared migration execution, reports DB, state DB, and UI-run registry ownership modules while preserving `apply_reports_db_migrations`, `apply_state_db_migrations`, and `apply_ui_run_registry_migrations`.
- [ ] Run the structural test and confirm failure because the private owners do not exist.
- [ ] Move schema statements and migration functions by database family; keep only compatibility imports in the facade.
- [ ] Run `tests/test_sqlite_migration_service.py`, schema-authority tests, and the structural test.
- [ ] Compare fresh SQLite schemas and migration responses against `HEAD`.
...
- [ ] Add a structural test requiring deterministic URL/title classifiers, observation evaluation/recovery, and generator workflow owners while preserving `qualify_publisher_inventory_candidates`.
- [ ] Run the structural test and confirm failure because the private owners do not exist.
- [ ] Move bodies without changing thresholds, branch order, reasons, logging, contracts, or candidate ordering.
- [ ] Run the complete candidate-quality suite and publisher-inventory orchestrator regressions.
- [ ] Compare canonical responses and runtime against `HEAD` on representative positive, rejection, and recovery fixtures.
...
- [ ] Add a structural test requiring models, transport, session/target resolution, dialog policy, and public operation owners while preserving current CDP exports.
- [ ] Run the structural test and confirm failure because the private owners do not exist.
- [ ] Move bodies with unchanged timeout, attachment, target selection, dialog, screenshot, print, and network behavior.
- [ ] Run CDP and browser-download affected suites.
- [ ] Compare deterministic fake-client call sequences, results, and runtime against `HEAD`.
...
- [ ] Add a structural test requiring parser, discovery/AJAX, landing inspection, and classification owners while preserving `discover_inventory_via_http`, `inspect_inventory_landing_pages`, and `HTTP_BROWSER_HEADERS`.
- [ ] Run the structural test and confirm failure because the private owners do not exist.
- [ ] Move bodies with unchanged requests, URL ordering, parsing, provenance, classification, and response contracts.
- [ ] Run publisher-inventory HTTP/parsing and workflow suites.
- [ ] Compare local HTTP fixture outputs and runtime against `HEAD`.
...
- [ ] Add a structural test requiring traversal, collection, interaction/wait, and HTTP supplement owners while preserving all workflow imports.
- [ ] Run the structural test and confirm failure because the private owners do not exist.
- [ ] Move bodies with unchanged navigation order, timeout budgets, click/scroll behavior, candidate ordering, browser close behavior, and fallback HTTP calls.
- [ ] Run publisher-inventory browser traversal, workflow, service, and orchestrator suites.
- [ ] Compare deterministic browser fixtures and an approved real publisher run against `HEAD`.
...
- [ ] Record per-target AST movement counts, changed-body explanations, facade-owned definitions, line counts, and dependency direction.
- [ ] Run Ruff, mypy, architecture/import gates, forbidden-patching gates, mutation/quality gates where configured, and the full default pytest suite.
- [ ] Run real affected-feature canaries using environment credentials only where the affected path actually requires them; do not call OpenAI or external APIs for paths that are deterministic/local.
- [ ] Commit the verified feature branch.
- [ ] Merge into `main` without staging, reverting, or committing the unrelated WordPress work already present there.
