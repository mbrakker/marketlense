...
> **Status:** Complete for the supported resolved-identity and exact-content-hash path
...
The persisted row stores `1` avoided acquisition action, browser launch, PDF
parse, OCR, extraction, and vector operation for that supported duplicate
fixture. It stores model calls, input/output tokens, and estimated cost as
`0` with attribution state `unavailable`: this fixture does not make a
provider call and must not infer a provider saving from elapsed test time.
...
| Provider-work span / reuse-resolution median | 90,295.028 ms | 31.224 ms |
...
The source-reuse suite passed 8 tests. The source-reuse benchmark suite passed
2 tests. The stale-owner fixture passed with no duplicate download and
