...
Before the artifact is retained, a deterministic source-display pass restores a
currency decimal or month/year qualifier only when the exact linked evidence
contains one unique source display. If a shortened currency value could refer
to two different retained decimal values, the entire affected public sentence
is omitted rather than inventing precision. This is a fail-closed preservation
rule: it never selects between competing facts, relaxes a quality rule, or
repairs a report manually. The LinkedIn publication and LinkedIn-regeneration
namespaces use their own zero-temperature execution policies because that
surface must restate retained evidence faithfully; schema, grounding, semantic,
editorial, and signed-readiness gates still run unchanged.
