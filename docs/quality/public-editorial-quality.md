...
Before the artifact is retained, a deterministic source-display pass preserves
...
and LinkedIn copy. If an abbreviated display could refer to two retained source
displays, the entire affected public sentence is omitted rather than inventing
precision. These fail-closed controls never select between competing facts,
relax a quality rule, or repair a report manually. The LinkedIn publication and
LinkedIn-regeneration namespaces use their own zero-temperature execution
policies because that surface must restate retained evidence faithfully;
schema, grounding, semantic, editorial, and signed-readiness gates still run
unchanged.
