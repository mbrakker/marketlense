...
- The latest reports/report-browser cards keep the same query and ordering logic but now use a fixed archive information stack (date, period, title, publisher, metrics, excerpt, CTA) with 4:3 media crops, 18px titles, muted 12px metadata, a longer archive-specific excerpt source, a 5-line reserved TLDR area on standard layouts with a tighter 3-line reserve on narrow layouts, inline digest CTAs spaced about 30px below the visible summary text, archive-only auto-inserted line breaks neutralized, and content-height cards that finish shortly below the link instead of stretching to the tallest card in the row
...
- `assets/css/theme.css` now includes a final premium-polish layer with explicit shared hooks (`.ml-card`, `.ml-chip`, `.ml-link-arrow`, `.ml-button`), subtle hero and briefing-band node motifs, a node-ended section rule, quieter shared card/button/link transitions, and explicit `prefers-reduced-motion` handling without changing any shortcode/query logic
- the site header/footer shell is now further refined in `assets/css/theme.css` so the two-row header reads as one integrated premium surface with a stronger brand lockup, calmer nav/current-state styling, a more attached archive-search row, polished request CTA states, and a lighter aligned footer surface/navigation treatment without changing information architecture or template hierarchy
- `functions.php` now versions theme assets with a `theme-version.filemtime` suffix so frontend CSS/JS changes invalidate stale browser caches automatically instead of relying on manual theme header bumps alone
- `assets/css/theme.css` now ends with a dedicated responsive premium pass at `1100px` and `782px`, tightening shell gutters/section spacing and rebalancing the hero, featured digest, signals, themes, publishers, reports, methodology cards, briefing band, and footer for tablet/mobile without changing shortcode/query behavior
- `assets/js/reveal.js` lowers its IntersectionObserver threshold and bottom root margin on `max-width: 782px` viewports so tall homepage bands, including Latest Reports, still become visible on mobile
- the final cleanup pass in `assets/css/theme.css` consolidates shared card/button/chip tokens, restores the missing hover treatment on signal cards, aligns editorial link/focus behavior, refreshes the briefing-band/footer control surfaces, and fixes the integrated header-search shell alignment without changing data/query/template logic
...
- `templates/archive-ml_report.html`, `templates/archive.html`, `templates/category.html`, `templates/taxonomy-ml_publisher.html`, and `templates/search.html` now route through the richer shortcode-based report browser instead of plain `core/query` grids.
- `templates/archive.html` intentionally mirrors the reports archive browser as a hierarchy fallback because the `ml_report` archive query is widened to include legacy digest `post` entries; that mixed post-type archive can fall back from the CPT-specific block template to the generic archive template in WordPress.
