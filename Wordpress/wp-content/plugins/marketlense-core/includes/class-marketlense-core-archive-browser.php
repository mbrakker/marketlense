...
final class Archive_Browser
{
    ...
    public function render(array $attrs, string $content_type): string
    {
        ...
        $intelligence_options = $definition['slug'] === 'report' ? $this->facet_public_intelligence_values($definition, $filters) : [];
                            ...
                            <details class="ml-report-filter-panel" open>
                                ...
                                <div class="ml-report-filter-body">
                                    ...
                                    <form class="ml-report-filter-form" method="get" action="<?php echo esc_url($archive_url); ?>" data-ml-live-filter-form>
                                        <span class="screen-reader-text" data-ml-filter-status aria-live="polite"></span>
                                        <?php $this->render_hidden_inputs($this->filter_args($filters, ['topic', 'publisher', 'period', 'region', 'intelligence'])); ?>
                                        <div class="ml-report-filter-grid">
                                            ...
                                            <?php if ($definition['slug'] === 'report') : $this->render_value_select('ml_intelligence_filter', 'ml_intelligence', __('Evidence modules', 'marketlense-core'), __('All reports', 'marketlense-core'), $filters['intelligence'], $intelligence_options); endif; ?>
                                        </div>
                                    </form>
                                </div>
                            </details>
    ...
    /** @return array{topic:string,publisher:string,period:string,region:string,intelligence:string,search:string} */
    private function selected_filters(): array
    {
        $intelligence = sanitize_key((string) ($_GET['ml_intelligence'] ?? ''));
        return ['topic' => sanitize_title((string) ($_GET['category'] ?? '')), 'publisher' => sanitize_title((string) ($_GET['ml_publisher'] ?? '')), 'period' => sanitize_text_field((string) ($_GET['ml_period'] ?? '')), 'region' => sanitize_text_field((string) ($_GET['ml_region'] ?? '')), 'intelligence' => $intelligence === 'public_intelligence' ? $intelligence : '', 'search' => sanitize_text_field((string) ($_GET['s'] ?? ''))];
    }
    ...
    /** @param array{slug:string,post_type:string|list<string>,schema_key:string,singular:string,plural:string,browser_label:string} $definition @param array{topic:string,publisher:string,period:string,region:string,intelligence?:string,search:string} $filters @return array<string,mixed> */
    private function query_args(array $definition, array $filters, int $per_page, int $paged, string $sort, string $exclude = ''): array
    {
        ...
        if ($definition['slug'] === 'report' && $exclude !== 'intelligence' && ($filters['intelligence'] ?? '') === 'public_intelligence') {
            $meta_query[] = ['key' => Meta::META_PUBLIC_INTELLIGENCE, 'value' => '1', 'compare' => '='];
        }
    ...
    private function facet_public_intelligence_values(array $definition, array $filters): array { $count = 0; foreach ($this->facet_ids($definition, $filters, 'intelligence') as $id) { if (trim((string) get_post_meta($id, Meta::META_PUBLIC_INTELLIGENCE, true)) === '1') { $count++; } } return $count > 0 ? [['value' => 'public_intelligence', 'label' => __('Reports with public intelligence', 'marketlense-core'), 'count' => $count]] : []; }
    ...
    private function render_value_select(string $id, string $name, string $label, string $all_label, string $selected, array $values): void { ?><label class="ml-report-filter-field" for="<?php echo esc_attr($id); ?>"><span><?php echo esc_html($label); ?></span><select id="<?php echo esc_attr($id); ?>" name="<?php echo esc_attr($name); ?>"><option value=""><?php echo esc_html($all_label); ?></option><?php foreach ($values as $item) : $display = (string) ($item['label'] ?? $item['value']); ?><option value="<?php echo esc_attr($item['value']); ?>" <?php selected($selected, $item['value']); ?>><?php echo esc_html(sprintf('%1$s (%2$d)', $display, $item['count'])); ?></option><?php endforeach; ?></select></label><?php }
    ...
    private function filter_args(array $filters, array $exclude): array { $args = ['category' => $filters['topic'], 'ml_publisher' => $filters['publisher'], 'ml_period' => $filters['period'], 'ml_region' => $filters['region'], 'ml_intelligence' => $filters['intelligence'] ?? '', 's' => $filters['search']]; foreach ($exclude as $key) { unset($args[$key === 'topic' ? 'category' : ($key === 'publisher' ? 'ml_publisher' : ($key === 'search' ? 's' : ($key === 'intelligence' ? 'ml_intelligence' : 'ml_' . $key)))]); } return $args; }
    ...
    private function render_active_filters(string $archive_url, array $filters, string $sort): void { $active = array_filter($filters); if ($active === []) { return; } ?><div class="ml-active-filters" aria-label="<?php esc_attr_e('Active filters', 'marketlense-core'); ?>"><span class="ml-active-filters-label"><?php esc_html_e('Selected', 'marketlense-core'); ?></span><?php foreach ($active as $key => $value) : ?><a class="ml-filter-chip" href="<?php echo esc_url(add_query_arg($this->filter_args($filters, [$key]), $archive_url)); ?>"><?php echo esc_html($this->active_filter_label((string) $key, (string) $value)); ?></a><?php endforeach; ?><a class="ml-filter-chip ml-filter-chip-clear" href="<?php echo esc_url($archive_url); ?>"><?php esc_html_e('Clear all', 'marketlense-core'); ?></a></div><?php }
    private function active_filter_label(string $key, string $value): string { if ($key === 'intelligence' && $value === 'public_intelligence') { return __('Evidence modules: public intelligence', 'marketlense-core'); } return ucfirst($key) . ': ' . $value; }
